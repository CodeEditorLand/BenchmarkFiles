...
export const Title = ({
	...
	return (
		<div class={`story-title ${offset && "offset"}`}>
			<h1>
				{title}
...
export const Example = ({
	...
	return (
		<div class={classNames("examples-item", isFullWidth && "w-full")}>
			<span
				class={classNames(
					"examples-item-title",
			...
			<div class="examples-item-content">{children}</div>
		</div>
	);
};
...
export const Examples = ({
	...
	return (
		<div class={classNames("examples", dark && "dark")}>
			<h2 class="examples-title">{title}</h2>
			<div class="examples-content">{children}</div>
			{!!footnote && <div class="examples-footnote">{footnote}</div>}
		</div>
	);
};
...
export function VariantsTable({
	...
	return (
		<div
			...
			<div class="z-1 relative mx-auto w-full max-w-[1200px] overflow-auto pr-8 pt-6">
				<table>
					<RowTitles titles={titles} />
					{columns.map((column) => (
						<tr class="p-2 pb-6 pr-6" key={column.props.variant}>
							<th
								class="p-2 pb-6 pr-6 text-left text-sm font-normal text-[#8F8F8F]"
			...
			{!isDark && (
				<div id="dark-variant" data-mode="dark" class="dark">
					<VariantsTable
						titles={titles}
						isDark={true}
...
export function RowTitles({ titles }: { titles: string[] }) {
	return (
		<tr>
			<th class="p-2 pb-6 pr-6 text-left text-sm font-normal text-[#8F8F8F]" />
			{titles.map((title) => (
				<th
					class="p-2 pb-6 pr-6 text-left text-sm font-normal text-[#8F8F8F]"
					key={title}>
...
export function createHydrateClient(opts: { transformer: DataTransformer }) {
	return function HydrateClient(props: {
		...
		return (
			<HydrationBoundary state={transformedState}>
				{children}
			</HydrationBoundary>
		);
	};
...
export function WithLayout<T extends Record<string, any>>({
	...
	return async <P extends "P" | "L">(
		p: P extends "P" ? PageProps : LayoutProps,
	) => {
		const h = headers();
		...
		if ("searchParams" in p && getData) {
			props =
				(await getData(
					buildLegacyCtx(h, cookies(), p.params, p.searchParams),
				)) ?? ({} as T);
		}
...
const getInitialProps = async (url: string) => {
	...
	const isEmbed =
		...
		(searchParams?.get("embedType") ?? null) !== null;
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <html
      ...
      <body
        class="dark:bg-darkgray-50 todesktop:!bg-transparent bg-subtle antialiased"
        style={
          isEmbed
            ? {
...
const getData =
	withAppDirSsr<inferSSRProps<typeof getServerSideProps>>(getServerSideProps);
...
export const generateMetadata = async ({
	...
	const legacyCtx = buildLegacyCtx(
		headers(),
		cookies(),
		params,
		searchParams,
...
export const generateMetadata = async ({
	...
	const props = await getData(
		legacyCtx as unknown as GetServerSidePropsContext,
	);
	...
	return await _generateMetadata(
		(t) =>
			`${
				booking?.uid && !!booking ? t("reschedule") : ""
			} ${title} | ${profileName}`,
...
export const generateMetadata = async ({
	...
	return await _generateMetadata(
		(t) =>
			`${
				rescheduleUid && !!booking ? t("reschedule") : ""
			} ${title} | ${profileName}`,
...
const getPageProps = async (context: GetServerSidePropsContext) => {
	...
	return redirect(
		`/apps/routing-forms/${pages.length ? pages.join("/") : ""}`,
	);
};
...
type AppPageType = {
	...
	default: ((props: any) => JSX.Element) &
		Pick<
			AppProps["Component"],
			"isBookingPage" | "getLayout" | "PageWrapper"
		>;
};
...
function getRoute(appName: string, pages: string[]) {
	...
	const appPage =
		routingConfig.layoutHandler || (routingConfig[mainPage] as AppPageType);
...
export const DatePicker = ({
	...
	return (
		<PrimitiveDatePicker
			...
			clearIcon={null}
			calendarIcon={<Calendar class="text-subtle h-5 w-5" />}
			value={date}
			minDate={minDate}
			disabled={disabled}
		...
		<div class="flex items-center gap-3">
			{icon && (
				<img
					src={icon}
					alt="cover"
					class={classNames("h-3.5 w-3.5", invertLogoOnDark(icon))}
				/>
			)}
			<span class={classNames("text-sm font-medium")}>{label}</span>
		</div>
	);
};
        ...
        <div class="flex items-center gap-3">
          {e.icon && (
            <img
              src={e.icon}
              alt="app-icon"
              class={classNames(e.icon.includes("-dark") && "dark:invert", "h-5 w-5")}
            />
          )}
          <span>{e.label}</span>
        </div>
      )}
      formatGroupLabel={(e) => <p class="text-default text-xs font-medium">{e.label}</p>}
      {...props}
    />
  );
}
...
const MinutesField = forwardRef<HTMLInputElement, Props>(
	({ label, ...rest }, ref) => {
		return (
			<div class="block sm:flex">
				{!!label && (
					<div class="min-w-48 mb-4 sm:mb-0">
						<label
							for={rest.id}
							class="text-default flex h-full items-center text-sm font-medium">
							{label}
						</label>
					</div>
				)}
				<div class="w-full">
					<div class="relative rounded-sm">
						...
						<div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
							<span class="text-subtle text-sm" id="duration">
								mins
							</span>
						</div>
					</div>
...
export const CheckedSelect = ({
	...
	return (
		<>
			...
			/>
			{value.map((option) => (
				<div key={option.value} class="border p-2 font-medium">
					<Avatar
						class="inline ltr:mr-2 rtl:ml-2"
						size="sm"
					...
					<X
						...
						class="text-subtle float-right mt-0.5 h-5 w-5 cursor-pointer"
					/>
				</div>
			))}
		</>
...
>({ className, ...props }: SelectProps<Option, IsMulti, Group>) {
	...
	return (
		<ReactSelect
				...
				colors: {
					...
					...(hasDarkTheme
						...
						: {
								...
								primary25:
									"rgba(244, 245, 246, var(--tw-bg-opacity))",
								/** Light Theme Ends */
							}),
				},
			})}
			...
			class={classNames("border-0 text-sm", className)}
			{...props}
		/>
	);
}
...
>({
	...
	const setHiddenInputValue = useCallback(
	...
	return (
		<div class={classNames("relative", remainingProps.className)}>
			<Select
				value={value}
				{...remainingProps}
				...
				{label && (
					<div class="min-w-48 mb-4 sm:mb-0">
						{React.createElement(
							...
							{
								className:
									"flex text-sm font-medium text-default",
								...(descriptionAsLabel
									? {}
									: {
											htmlFor: rest.id,
							...
							<>
								<div class="flex h-5 items-center">
									<input
										...
										ref={ref}
										type="checkbox"
										class="text-emphasis focus:ring-emphasis dark:text-muted border-default bg-default h-4 w-4 rounded"
									/>
								</div>
								<span class="ms-2 text-sm">{description}</span>
							</>,
						)}
						{informationIconText && (
							<InfoBadge content={informationIconText} />
						)}
					</div>
				</div>
			</div>
...
export default function SettingInputContainer({
  ...
  return (
    <div class="space-y-3">
      <div class="block sm:flex">
        <div class="min-w-48 mb-4 sm:mb-0">
          <label for={htmlFor} class="text-default mt-1 flex text-sm font-medium">
            <Icon class="text-subtle mt-0.5 h-4 w-4 ltr:mr-2 rtl:ml-2" />
            {label}
          </label>
        </div>
        <div class="w-full flex-grow">{Input}</div>
      </div>
    </div>
  );
}
...
export const UsernameAvailabilityField = ({
	...
	const UsernameAvailability = getUsernameAvailabilityComponent(
		!IS_SELF_HOSTED && !user.organization?.id,
	);
...
const UsernameTextfield = (
	...
	const ActionButtons = () => {
		return usernameIsAvailable && currentUsername !== inputUsernameValue ? (
			<div class="relative bottom-[6px] me-2 ms-2 flex flex-row space-x-2">
				<Button
					type="button"
			...
			<div class="flex rounded-md">
				<div class="relative w-full">
					...
					{currentUsername !== inputUsernameValue && (
						<div class="absolute right-[2px] top-6 flex flex-row">
							<span class={classNames("mx-2 py-3.5")}>
								{usernameIsAvailable ? (
									<Check class="relative bottom-[6px] h-4 w-4" />
								) : (
									<></>
								)}
							</span>
				...
				<div class="mt-7 hidden md:inline">
					<ActionButtons />
				</div>
			</div>
			{markAsError && (
			...
			<Dialog open={openDialogSaveUsername}>
				<DialogContent
					...
					<div class="flex flex-row">
						<div class="mb-4 w-full pt-1">
							<div class="bg-subtle flex w-full flex-wrap gap-6 rounded-sm px-2 py-3 text-sm">
								<div>
									<p class="text-subtle">
										{t("current_username")}
									</p>
									<p
										class="text-emphasis mt-1"
...
const PremiumTextfield = (props: ICustomUsernameProps) => {
	...
	let paymentMsg = currentUsername ? null : (
		<span class="text-xs text-orange-400">
			You need to reserve your premium username for{" "}
	...
	if (recentAttemptPaymentStatus && recentAttemptPaymentStatus !== "paid") {
		paymentMsg = (
			<span class="text-sm text-red-500">
				Your payment could not be completed. Your username is still not
				reserved
			</span>
		);
	}
		...
		<div>
				...
				<div class="relative w-full">
					<Input
						...
						class={classNames(
							...
							markAsError
								...
								: "border-l-default",
							disabled
								? "bg-subtle text-muted focus:border-0"
								: "",
						)}
						value={inputUsernameValue}
						onChange={(event) => {
							...
							if (
								...
								router.replace(
									`${pathname}?${_searchParams.toString()}`,
								);
							}
							setInputUsernameValue(event.target.value);
						}}
					...
					<div class="absolute right-2 top-0 flex flex-row">
						<span
							...
							{isInputUsernamePremium ? (
								<StarSolid class="mt-[2px] h-4 w-4 fill-orange-400" />
							) : (
								<></>
							)}
							{!isInputUsernamePremium && usernameIsAvailable ? (
								<Check class="mt-[2px] h-4 w-4" />
							) : (
								<></>
							)}
						</span>
			...
			<Dialog open={openDialogSaveUsername}>
				<DialogContent
						...
						<>
							{usernameChangeCondition &&
								usernameChangeCondition ===
									...
									<p class="text-default mb-4 text-sm">
										{t(
											"change_username_standard_to_premium",
										)}
									</p>
								)}
						</>
					...
					<div class="flex flex-row">
						<div class="mb-4 w-full px-4 pt-1">
							<div class="bg-subtle flex w-full flex-wrap rounded-sm py-3 text-sm">
...
const EditableHeading = function EditableHeading({
	...
	return (
		<div
			class="group pointer-events-none relative truncate sm:pointer-events-auto"
			onClick={enableEditing}>
			<div class="flex cursor-pointer items-center">
				<label class="min-w-8 relative inline-block">
					<span class="whitespace-pre text-xl tracking-normal text-transparent">
						{value}&nbsp;
					</span>
					{!isEditing && isReady && (
						<Edit2 class=" text-subtle group-hover:text-subtle -mt-px ml-1 inline  h-3 w-3" />
					)}
					<input
						{...passThroughProps}
						type="text"
						...
						onFocus={(e) => {
							setIsEditing(true);
							passThroughProps.onFocus &&
								passThroughProps.onFocus(e);
						}}
						onBlur={(e) => {
							setIsEditing(false);
							passThroughProps.onBlur &&
  ...
  return (
    <div class="-ml-2">
      <button
        type="button"
        {...props}
        class="text-md hover:bg-emphasis hover:text-emphasis text-default flex items-center rounded-sm px-2 py-1 text-sm font-medium">
        <props.Icon class="text-subtle h-4 w-4 ltr:mr-2 rtl:ml-2" />
        {props.children}
      </button>
    </div>
  );
...
export default function AuthContainer(props: React.PropsWithChildren<Props>) {
  return (
    <div class="bg-subtle dark:bg-darkgray-50 flex min-h-screen flex-col justify-center py-12 sm:px-6 lg:px-8">
      <HeadSeo title={props.title} description={props.description} />
      {props.showLogo && <Logo small={true} inline={false} class="mx-auto mb-auto" />}

      <div class={classNames(props.showLogo ? "text-center" : "", "sm:mx-auto sm:w-full sm:max-w-md")}>
        {props.heading && <h2 class="font-cal text-emphasis text-center text-3xl">{props.heading}</h2>}
      </div>
      {props.loading && (
        <div class="bg-muted absolute z-50 flex h-screen w-full items-center">
          <Loader />
        </div>
      )}
      <div class="mb-auto mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div class="bg-default dark:bg-muted border-subtle mx-2 rounded-md border px-4 py-10 sm:px-10">
          {props.children}
        </div>
        <div class="text-default mt-8 text-center text-sm">{props.footerText}</div>
      </div>
    </div>
  );
}
		...
		try {
			...
			if (response.status === 200) {
				...
				const textBlob = new Blob(
					[body.backupCodes.map(formatBackupCode).join("\n")],
					{
						type: "text/plain",
					},
				);
	...
	async function handleEnable(
		...
		try {
			...
			if (body.error === ErrorCode.IncorrectTwoFactorCode) {
				setErrorMessage(
					`${t("code_is_incorrect")} ${t("please_try_again")}`,
				);
			} else {
				setErrorMessage(t("something_went_wrong"));
			}
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent
				...
				<WithStep step={SetupStep.DisplayQrCode} current={step}>
					<>
						...
						<p
							...
							class="mb-4 text-center font-mono text-xs">
							{secret}
						</p>
					</>
				</WithStep>
				<WithStep step={SetupStep.DisplayBackupCodes} current={step}>
					<>
						<div class="mt-5 grid grid-cols-2 gap-1 text-center font-mono md:pl-10 md:pr-10">
				...
				<Form handleSubmit={handleEnable} form={form}>
					<WithStep step={SetupStep.EnterTotpCode} current={step}>
						<div class="-mt-4 pb-2">
							<TwoFactor center={true} />

							{errorMessage && (
								<p
									data-testid="error-submitting-code"
					...
					<DialogFooter class="mt-8" showDivider={true}>
						{step !== SetupStep.DisplayBackupCodes ? (
							<Button
								color="secondary"
								onClick={() => {
						...
						<WithStep
							step={SetupStep.ConfirmPassword}
							current={step}>
							<Button
								type="submit"
								class="me-2 ms-2"
...
const DisableTwoFactorAuthModal = ({
	...
	async function handleDisable({
		...
		try {
			const response = await TwoFactorAuthAPI.disable(
				password,
				totpCode,
				backupCode,
			);
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent
				...
				<Form form={form} handleSubmit={handleDisable}>
					<div class="mb-8">
						{!disablePassword && (
							<PasswordField
								...
								class="border-default mt-1 block w-full rounded-md border px-3 py-2 text-sm focus:border-black focus:outline-none focus:ring-black"
							/>
						)}
						{twoFactorLostAccess ? (
					...
					<DialogFooter showDivider={true} class="relative mt-5">
						<Button
							color="minimal"
							class="mr-auto"
							onClick={() => {
    ...
    <div>
      <ShellMain heading="Enterprise" subtitle={t("enterprise_description")}>
        <UpgradeTip
          ...
          buttons={
            <div class="space-y-2 rtl:space-x-reverse sm:space-x-2">
              <ButtonGroup>
			...
			<DialogContent>
				...
				<WithStep step={SetupStep.ConfirmPassword} current={step}>
					<form onSubmit={handleSetup}>
						<div class="mb-4">
							<label
								for="password"
								class="text-default mt-4 block text-sm font-medium">
								{t("password")}
							</label>
							<div class="mt-1">
								<input
									...
									class="border-default block w-full rounded-sm text-sm"
								/>
							</div>
				...
				<WithStep step={SetupStep.DisplayQrCode} current={step}>
					<>
						...
						<p class="text-center font-mono text-xs">{secret}</p>
					</>
				</WithStep>
				<Form handleSubmit={handleEnable} form={form}>
					<WithStep step={SetupStep.EnterTotpCode} current={step}>
...
const TwoFactorAuthSection = ({
	...
	return (
		<>
			<div class="flex flex-col justify-between pl-2 pt-9 sm:flex-row">
				<div>
					<div class="flex flex-row items-center">
						<h2 class="font-cal text-emphasis text-lg font-medium leading-6">
							{t("2fa")}
						</h2>
						<Badge
							class="ml-2 text-xs"
				...
				<div class="mt-5 sm:mt-0 sm:self-center">
					<Button
						type="submit"
						color="secondary"
						onClick={() =>
...
const TwoFactorModalHeader = ({
	title,
	description,
}: { title: string; description: string }) => {
	return (
		<div class="mb-4 sm:flex sm:items-start">
			<div class="bg-brand text-brandcontrast dark:bg-darkmodebrand dark:text-darkmodebrandcontrast mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-opacity-5 sm:mx-0 sm:h-10 sm:w-10">
				<Shield class="text-inverted h-6 w-6" />
			</div>
			<div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
				<h3
					class="font-cal text-emphasis text-lg font-medium leading-6"
					id="modal-title">
					{title}
				</h3>
...
const ChangePasswordSection = () => {
	...
	const errorMessages: { [key: string]: string } = {
		...
		[ErrorCode.NewPasswordMatchesOld]: t(
		...
		try {
			...
			setErrorMessage(
				errorMessages[body.error] ||
					`${t("something_went_wrong")}${t("please_try_again")}`,
			);
		} catch (err) {
	...
	return (
		<>
			<form
				class="divide-subtle divide-y lg:col-span-9"
				onSubmit={changePasswordHandler}>
				<div class="py-6 lg:pb-5">
					<div class="my-3">
						<h2 class="font-cal text-emphasis text-lg font-medium leading-6">
							{t("change_password")}
						</h2>
					...
					<div class="flex flex-col space-y-2 sm:flex-row sm:space-y-0">
						<div class="w-full ltr:mr-2 rtl:ml-2 sm:w-1/2">
							<label
								for="current_password"
								class="text-default block text-sm font-medium">
								{t("current_password")}
							</label>
							<div class="mt-1">
								<input
					...
					<div class="flex py-8 sm:justify-end">
...
const DisableUserImpersonation = ({
			...
			<div class="flex flex-col justify-between pl-2 pt-9 sm:flex-row">
				<div>
					<div class="flex flex-row items-center">
						...
						<Badge
							class="ml-2 text-xs"
							variant={disableImpersonation ? "gray" : "success"}>
							{disableImpersonation
								? t("disabled")
								: t("enabled")}
						</Badge>
				...
				<div class="mt-5 sm:mt-0 sm:self-center">
					<Button
						...
						{disableImpersonation ? t("enable") : t("disable")}
					</Button>
				</div>
			</div>
		</>
...
export default function TwoFactor({ center = true, autoFocus = true }) {
  ...
  return (
    <div class={center ? "mx-auto !mt-0 max-w-sm" : "!mt-0 max-w-sm"}>
      <Label class="mt-4">{t("2fa_code")}</Label>

      <p class="text-subtle mb-4 text-sm">{t("2fa_enabled_instructions")}</p>
...
export default function TwoFactor({ center = true }) {
  ...
  return (
    <div class={center ? "mx-auto !mt-0 max-w-sm" : "!mt-0 max-w-sm"}>
      <Label class="mt-4">{t("backup_code")}</Label>

      <p class="text-subtle mb-4 text-sm">{t("backup_code_instructions")}</p>
...
export default function AdminLayout({
  ...
  return (
    <SettingsLayout {...rest}>
      <div class="divide-subtle mx-auto flex max-w-4xl flex-row divide-y">
        <div class={isAppsPage ? "min-w-0" : "flex flex-1 [&>*]:flex-1"}>
...
export default function AdminLayout({
  ...
  return (
    <SettingsLayout {...rest}>
      <div class="divide-subtle bg-default mx-auto flex max-w-4xl flex-row divide-y">
        <div class={isAppsPage ? "min-w-0" : "flex flex-1 [&>*]:flex-1"}>
          <ErrorBoundary>{children}</ErrorBoundary>
        </div>
      </div>
...
function SkeletonLoader() {
	return (
		<ul class="divide-subtle border-subtle bg-default animate-pulse divide-y rounded-md border sm:overflow-hidden">
			<SkeletonItem />
			<SkeletonItem />
			<SkeletonItem />
		</ul>
...
function SkeletonItem() {
	return (
		<li class="group flex w-full items-center justify-between px-4 py-4 sm:px-6">
			<div class="flex-grow truncate text-sm">
				<div class="flex">
					<div class="flex flex-col space-y-2">
						<SkeletonText class="h-5 w-16" />
						<SkeletonText class="h-4 w-32" />
					</div>
				</div>
			</div>
			<div class="mt-4 hidden flex-shrink-0 sm:ml-5 sm:mt-0 lg:flex">
				<div class="flex justify-between space-x-2 rtl:space-x-reverse">
					<SkeletonText class="h-6 w-16" />
					<SkeletonText class="h-6 w-32" />
				</div>
			</div>
		</li>
	);
...
type BookingItemProps = BookingItem & {
	...
	recurringInfo:
		| RouterOutputs["viewer"]["bookings"]["get"]["recurringInfo"][number]
		| undefined;
	loggedInUser: {
...
function BookingListItem(booking: BookingItemProps) {
	...
	const { userId, userTimeZone, userTimeFormat, userEmail } =
	...
	const [viewRecordingsDialogIsOpen, setViewRecordingsDialogIsOpen] =
	...
	const chargeCardActions: ActionType[] = [
		{
			id: "charge_card",
			label: cardCharged
				? t("no_show_fee_charged")
				: t("collect_no_show_fee"),
	...
	if (isTabRecurring && isRecurring) {
		bookedActions = bookedActions.filter(
			(action) => action.id !== "edit_booking",
		);
	}
	...
	const recurringDates = booking.recurringInfo?.bookings[
	...
	const buildBookingLink = () => {
		...
		if (booking.attendees[0])
			urlSearchParams.set("email", booking.attendees[0].email);
	...
	const showViewRecordingsButton = !!(
		booking.isRecorded &&
		isPast &&
		isConfirmed
	);
	...
	const showRecordingActions: ActionType[] = [
		{
			id: "view_recordings",
			label: showCheckRecordingButton
				? t("check_for_recordings")
				: t("view_recordings"),
			onClick: () => {
	...
	return (
		<>
			...
			<Dialog
				open={rejectionDialogIsOpen}
				onOpenChange={setRejectionDialogIsOpen}>
				<DialogContent
			...
			<tr
				...
				class="hover:bg-muted group flex flex-col sm:flex-row">
				<td class="hidden align-top ltr:pl-6 rtl:pr-6 sm:table-cell sm:min-w-[12rem]">
					<Link href={bookingLink}>
						<div class="cursor-pointer py-4">
							<div class="text-emphasis text-sm leading-6">
								{startTime}
							</div>
									...
									{(provider?.label ||
										...
										locationToDisplay.startsWith(
											...
											<a
												...
												rel="noreferrer"
												class="text-sm leading-6 text-blue-600 hover:underline dark:text-blue-400">
							...
							{booking.eventType?.team && (
								<Badge class="ltr:mr-2 rtl:ml-2" variant="gray">
				...
				<td class={`w-full px-4${isRejected ? " line-through" : ""}`}>
					<Link href={bookingLink}>
						{/* Time and Badges for mobile */}
						<div class="w-full pb-2 pt-4 sm:hidden">
							<div class="flex w-full items-center justify-between sm:hidden">
								<div class="text-emphasis text-sm leading-6">
									{startTime}
								</div>
								<div class="text-subtle pr-2 text-sm">
									{formatTime(
										booking.startTime,
										userTimeFormat,
										userTimeZone,
							...
							{isPending && (
								<Badge
									class="ltr:mr-2 rtl:ml-2 sm:hidden"
									variant="orange">
									{t("unconfirmed")}
								</Badge>
							)}
							...
							{recurringDates !== undefined && (
								<div class="text-muted text-sm sm:hidden">
									<RecurringBookingsTooltip
										userTimeFormat={userTimeFormat}
										userTimeZone={userTimeZone}
										booking={booking}
						...
						<div class="cursor-pointer py-4">
							<div
								...
								{paymentAppData.enabled &&
									...
									booking.payment.length && (
										<Badge
											class="me-2 ms-2 hidden sm:inline-flex"
											variant="orange">
											{t("pending_payment")}
										</Badge>
									)}
							...
							{booking.description && (
								<div
									class="max-w-10/12 sm:max-w-32 md:max-w-52 xl:max-w-80 text-default truncate text-sm"
							...
							{isCancelled && booking.rescheduled && (
								<div class="mt-2 inline-block md:hidden">
									<RequestSentMessage />
								</div>
							)}
						</div>
				...
				<td class="flex w-full justify-end py-4 pl-4 text-right text-sm font-medium ltr:pr-4 rtl:pl-4 sm:pl-0">
	...
	return (
		(booking.recurringInfo &&
			...
			(booking.listingStatus === "recurring" ||
				...
				<div class="underline decoration-gray-400 decoration-dashed underline-offset-2">
					<div class="flex">
						<Tooltip
							content={recurringDates.map((aDate, key) => {
								const pastOrCancelled =
									aDate < now ||
									booking.recurringInfo?.bookings[
							...
							<div class="text-default">
								<RefreshCcw
									strokeWidth="3"
									class="text-muted float-left mr-1 mt-1.5 inline-block h-3 w-3"
								/>
...
const FirstAttendee = ({
	...
	return user.email === currentEmail ? (
		<div class="inline-block">{t("you")}</div>
	) : (
		<a
			key={user.email}
...
const DisplayAttendees = ({
	...
	return (
		<div class="text-emphasis text-sm">
			...
			{attendees.length > 1 && (
				<>
					<div class="text-emphasis inline-block text-sm">
						&nbsp;{t("and")}&nbsp;
					</div>
					{attendees.length > 2 ? (
						<Tooltip
...
export default function CancelBooking(props: Props) {
  ...
  return (
    <>
      {error && (
        <div class="mt-8">
          <div class="bg-error mx-auto flex h-12 w-12 items-center justify-center rounded-full">
            <X class="h-6 w-6 text-red-600" />
          </div>
          <div class="mt-3 text-center sm:mt-5">
            <h3 class="text-emphasis text-lg font-medium leading-6" id="modal-title">
              {error}
            </h3>
          </div>
        </div>
      ...
      {!error && (
        <div class="mt-5 sm:mt-6">
          <label class="text-default font-medium">{t("cancellation_reason")}</label>
          <TextArea
            ...
            rows={3}
          />
          <div class="flex flex-col-reverse rtl:space-x-reverse ">
            <div class="ml-auto flex w-full space-x-4 ">
              <Button
                class="ml-auto"
                color="secondary"
	...
	return (
		<form
			...
			<RadioGroup.Root
				...
				class="grid grid-rows-2 gap-4 md:grid-cols-2 md:grid-rows-1"
				onValueChange={(value) => {
					onChange(value);
					setValue(value);
				}}>
				<RadioGroup.Item value="FREE">
					<div
						...
						<h2 class="font-cal text-emphasis text-xl">
							{t("agplv3_license")}
						</h2>
						<p class="font-medium text-green-800">
							{t("free_license_fee")}
						</p>
						<p class="text-subtle">{t("forever_open_and_free")}</p>
						<ul class="text-subtle ml-4 list-disc text-left text-xs">
							<li>
				...
				<RadioGroup.Item value="EE" disabled={true}>
					<Link href="https://cal.com/sales" target="_blank">
						<div
							class={classNames(
								"bg-default h-full cursor-pointer space-y-2 rounded-md border p-4",
							)}>
							<h2 class="font-cal text-emphasis text-xl">
								{t("custom_plan")}
							</h2>
							...
							<p class="text-subtle">Build on top of Cal.com</p>
							<ul class="text-subtle ml-4 list-disc text-left text-xs">
								<li>
									{t("no_need_to_keep_your_code_open_source")}
								</li>
								<li>{t("repackage_rebrand_resell")}</li>
								<li>
									{t("a_vast_suite_of_enterprise_features")}
								</li>
							</ul>
						</div>
					</Link>
...
const EnterpriseLicense = (
	...
	return (
		<FormProvider {...formMethods}>
			<form
				{...rest}
				class="bg-default space-y-4 rounded-md px-8 py-10"
				...
				<div>
					...
					<div class="relative flex justify-center">
						<hr class="border-subtle my-8 w-full border-[1.5px]" />
						<span class="bg-default absolute mt-[22px] px-3.5 text-sm">
							OR
						</span>
					</div>
					{t("already_have_key")}
...
export const AdminUserContainer = (
	props: React.ComponentProps<typeof AdminUser> & { userCount: number },
) => {
	const { t } = useLocale();
...
export const AdminUser = (props: {
	...
	const formSchema = z.object({
		username: z.string().refine((val) => val.trim().length >= 1, {
			message: t("at_least_characters", { count: 1 }),
		}),
						...
						render={({ field: { onBlur, onChange, value } }) => (
							<>
								<Label
									...
									{longWebsiteUrl && (
										<small class="items-centerpx-3 bg-subtle border-default text-subtle mt-2 inline-flex rounded-t-md border border-b-0 px-3 py-1">
											{
												process.env
								...
								<TextField
									addOnLeading={
										!longWebsiteUrl && (
											<span class="text-subtle inline-flex items-center rounded-none px-3 text-sm">
												{
													process.env
														.NEXT_PUBLIC_WEBSITE_URL
												}
					...
					<Controller
						...
						render={({ field: { onBlur, onChange, value } }) => (
							<TextField
								...
								onChange={(e) => onChange(e.target.value)}
								color={
									formMethods.formState.errors.full_name
										? "warn"
										: ""
								}
								type="text"
...
const StepDone = (props: {
	...
	return (
		<form
			...
			<div class="min-h-36 my-6 flex flex-col items-center justify-center">
				<div class="dark:bg-default flex h-[72px] w-[72px] items-center justify-center rounded-full bg-gray-600">
					<Check class="text-inverted dark:bg-default dark:text-default inline-block h-10 w-10" />
				</div>
				<div class="max-w-[420px] text-center">
					<h2 class="mb-1 mt-6 text-lg font-medium dark:text-gray-300">
						{t("all_done")}
					</h2>
				</div>
			</div>
...
export const EditLocationDialog = (props: ISetLocationDialog) => {
		...
		locationLink: z
			...
			.superRefine((val, ctx) => {
				if (
					...
					if (!valid) {
						const sampleUrl =
							eventLocationType.organizerInputPlaceholder;
						ctx.addIssue({
							code: z.ZodIssueCode.custom,
							message: `Invalid URL for ${
		...
		(location: { type: EventLocationType["type"]; address?: string }) => {
			if (location.type === LocationType.InPerson) {
				return (
					...
					location.address === selectedAddrValue
				);
			} else {
							...
							<Controller
								...
								render={() => (
									<CheckboxField
										...
										defaultChecked={
											defaultLocation?.displayLocationPublicly
										}
										description={t(
	...
	return (
		<Dialog
			open={isOpenDialog}
			onOpenChange={(open) => setShowLocationModal(open)}>
			<DialogContent>
				<div class="flex flex-row space-x-3">
					<div class="bg-subtle mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full sm:mx-0 sm:h-10 sm:w-10">
						<MapPin class="text-emphasis h-6 w-6" />
					</div>
					<div class="w-full">
						<div class="mt-3 text-center sm:mt-0 sm:text-left">
							<h3
								class="text-emphasis text-lg font-medium leading-6"
								id="modal-title">
								{t("edit_location")}
							</h3>
							{!booking && (
								<p class="text-default text-sm">
									<Trans i18nKey="cant_find_the_right_video_app_visit_our_app_store">
										Can&apos;t find the right video app?
										Visit our
										<Link
						...
						{booking && (
							<>
								<p class="text-emphasis mb-2 ml-1 mt-6 text-sm font-bold">
									{t("current_location")}:
								</p>
								<p class="text-emphasis mb-2 ml-1 text-sm">
									{getHumanReadableLocationValue(
										booking.location,
										t,
									)}
...
export const RescheduleDialog = (props: IRescheduleDialog) => {
	...
	return (
		<Dialog open={isOpenDialog} onOpenChange={setIsOpenDialog}>
			<DialogContent enableOverflow={true}>
				<div class="flex flex-row space-x-3">
					<div class="bg-subtle flex h-10 w-10 flex-shrink-0 justify-center rounded-full ">
						<Clock class="m-auto h-6 w-6" />
					</div>
					<div class="pt-1">
						...
						<p class="text-emphasis mb-2 mt-6 text-sm font-bold">
...
import { AlertTriangle, CreditCard } from "@calcom/ui/components/icon";
	...
	return (
		<Dialog open={isOpenDialog} onOpenChange={setIsOpenDialog}>
			<DialogContent>
				<div class="flex flex-row space-x-3">
					<div class=" bg-subtle flex h-10 w-10 flex-shrink-0 justify-center rounded-full">
						<CreditCard class="m-auto h-6 w-6" />
					</div>
					<div class="pt-1">
						...
						<DialogFooter>
							...
							<Button
								data-testid="send_request"
								disabled={
									chargeCardMutation.isPending || chargeError
								}
								onClick={() =>
...
const CreateEventsOnCalendarSelect = (
	props: ICreateEventsOnCalendarSelectProps,
) => {
		...
		<>
			<div class="mt-6 flex flex-row">
				<div class="w-full">
					<label
						for="createEventsOn"
						class="text-default flex text-sm font-medium">
						{t("create_events_on")}
					</label>
					<div class="mt-2">
	...
	return (
		<>
			<div class="flex flex-row items-center p-4">
				<img src={logo} alt={name} class="m-1 h-8 w-8" />
				<div class="mx-4">
					<p class="font-sans text-sm font-bold leading-5">
						{name}
              ...
              {t("default")}
					...
					<div class="fle-row flex">
						<span
							title={externalId}
							class="max-w-44 text-subtle mt-1 overflow-hidden text-ellipsis whitespace-nowrap font-sans text-sm">
							{externalId}{" "}
						</span>
					</div>
				</div>
          ...
          {t("edit")}
			...
			<div class="border-subtle h-[1px] w-full border-b" />
			<div>
				<ul class="p-4">
...
import { SkeletonAvatar, SkeletonButton, SkeletonText } from "@calcom/ui";

export function StepConnectionLoader() {
	return (
		<ul class="bg-default divide-subtle border-subtle divide-y rounded-md border p-0 dark:bg-black">
			{Array.from({ length: 4 }).map((_item, index) => {
				return (
					<li
						class="flex w-full flex-row justify-center border-b-0 py-6"
						key={index}>
						<SkeletonAvatar class="mx-6 h-8 w-8 px-4" />
						<SkeletonText class="ml-1 mr-4 mt-3 h-5 w-full" />
						<SkeletonButton class="mr-6 h-8 w-20 rounded-md p-5" />
					</li>
				);
			})}
		</ul>
		...
		user.organization && user.organization.id
			? {
					...(user.organization as Ensure<
						typeof user.organization,
						"id"
					>),
					slug: user.organization.slug || null,
					requestedSlug:
						user.organization.metadata?.requestedSlug || null,
				}
			: null;
	return (
		<form onSubmit={onSubmit}>
			<div class="flex flex-row items-center justify-start rtl:justify-end">
				{user && (
					<UserAvatar
						size="lg"
						user={user}
				...
				<input
					...
					id="avatar"
					placeholder="URL"
					class="border-default focus:ring-empthasis mt-1 block w-full rounded-sm border px-3 py-2 text-sm focus:border-gray-800 focus:outline-none"
			...
			<fieldset class="mt-8">
				<Label class="text-default mb-2 block text-sm font-medium">
					{t("about")}
				</Label>
				<Editor
					getText={() =>
				...
				/>
				<p class="text-default mt-2 font-sans text-sm font-normal">
			...
			<Button
				EndIcon={ArrowRight}
				type="submit"
				class="mt-8 w-full items-center justify-center">
				{t("finish")}
			</Button>
		</form>
	);
...
const SetupAvailability = (props: ISetupAvailabilityProps) => {
	...
	const scheduleId =
		defaultScheduleId === null ? undefined : defaultScheduleId;
	...
	const createSchedule =
		trpc.viewer.availability.schedule.create.useMutation(mutationOptions);
	const updateSchedule =
		trpc.viewer.availability.schedule.update.useMutation(mutationOptions);
	return (
		<Form
			class="bg-default dark:text-inverted text-emphasis w-full [--cal-brand-accent:#fafafa] dark:bg-opacity-5"
			...
			<div>
				<Button
					...
					type="submit"
					class="mt-2 w-full justify-center p-2 text-sm sm:mt-8"
					disabled={availabilityForm.formState.isSubmitting}>
					{t("next_step_text")}{" "}
					<ArrowRight class="ml-2 h-4 w-4 self-center" />
				</Button>
			</div>
		</Form>
	);
...
import { telemetryEventTypes, useTelemetry } from "@calcom/lib/telemetry";
import { trpc } from "@calcom/trpc/react";
import { Button, Input, TimezoneSelect } from "@calcom/ui";
import { ArrowRight } from "@calcom/ui/components/icon";
		...
		<form onSubmit={onSubmit}>
			<div class="space-y-6">
				...
				<div class="w-full">
					<label
						for="name"
						class="text-default mb-2 block text-sm font-medium">
						{t("full_name")}
					</label>
					<Input
						{...register("name", {
				...
				<div class="w-full">
					...
					<TimezoneSelect
						...
						class="mt-2 w-full rounded-md text-sm"
					/>

					<p class="text-subtle mt-3 flex flex-row font-sans text-xs leading-tight">
						{t("current_time")}{" "}
						{dayjs()
							.tz(selectedTimeZone)
							.format("LT")
			...
			<Button
				type="submit"
				class="mt-8 flex w-full flex-row justify-center"
				disabled={mutation.isPending}>
				{t("next_step_text")}
				<ArrowRight
...
const ConnectedCalendars = (props: IConnectCalendarsProps) => {
	...
	const destinationCalendar =
		queryConnectedCalendars.data?.destinationCalendar;
	return (
		<>
			...
			{!queryConnectedCalendars.isPending &&
				...
				firstCalendar.integration.logo && (
					<>
						<List class="bg-default border-subtle rounded-md border p-0 dark:bg-black ">
			...
			{firstCalendar === undefined &&
				...
				queryIntegrations.data.items.length > 0 && (
					<List class="bg-default divide-subtle border-subtle mx-1 divide-y rounded-md border p-0 dark:bg-black sm:mx-0">
						{queryIntegrations.data &&
...
const ConnectedVideoStep = (props: ConnectedAppStepProps) => {
	const { nextStep } = props;
	const { data: queryConnectedVideoApps, isPending } =
	...
	return (
		<>
			{!isPending && (
				<List class="bg-default  border-subtle divide-subtle scroll-bar mx-1 max-h-[45vh] divide-y !overflow-y-scroll rounded-md border p-0 sm:mx-0">
...
type MemberType = Pick<
	MembersType[number],
	"id" | "name" | "bio" | "username" | "organizationId"
> & {
	safeBio: string | null;
	bookerUrl: string;
};
...
const Member = ({
	member,
	teamName,
}: { member: MemberType; teamName: string | null }) => {
	const routerQuery = useRouterQuery();
	const { t } = useLocale();
	const isBioEmpty =
		!member.bio || !member.bio.replace("<p><br></p>", "").length;
	...
	return (
		<Link
			key={member.id}
			href={{
				pathname: `${member.bookerUrl}/${member.username}`,
				query: queryParamsToForward,
			}}>
			<div class="sm:min-w-80 sm:max-w-80 bg-default hover:bg-muted border-subtle group flex min-h-full flex-col space-y-2 rounded-md border p-4 hover:cursor-pointer">
				<UserAvatar size="md" user={member} />
				<section class="mt-2 line-clamp-4 w-full space-y-1">
					<p class="text-default font-medium">{member.name}</p>
					<div class="text-subtle line-clamp-3 overflow-ellipsis text-sm font-normal">
						{isBioEmpty ? (
							t("user_from_team", {
								user: member.name,
								team: teamName,
							...
							<>
								<div
									class="  text-subtle break-words text-sm [&_a]:text-blue-500 [&_a]:underline [&_a]:hover:text-blue-600"
									dangerouslySetInnerHTML={{
										__html: md.render(
											markdownToSafeHTML(member.bio),
										),
									}}
								/>
							</>
...
const Members = ({
	members,
	teamName,
}: { members: MemberType[]; teamName: string | null }) => {
	...
	return (
		<section
			...
			class="lg:min-w-lg mx-auto flex min-w-full max-w-5xl flex-wrap justify-center gap-x-6 gap-y-6">
			{members.map((member) => {
				return (
...
export default function AddToHomescreen() {
  ...
  return closeBanner ? null : (
    <div class="fixed inset-x-0 bottom-0 pb-2 sm:hidden sm:pb-5">
      <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
        <div class="rounded-lg p-2 shadow-lg sm:p-3" style={{ background: "#2F333D" }}>
          <div class="flex flex-wrap items-center justify-between">
            <div class="flex w-0 flex-1 items-center">
              <span class="bg-brand text-brandcontrast dark:bg-darkmodebrand dark:text-darkmodebrandcontrast flex rounded-lg bg-opacity-30 p-2">
                <svg
                  class="h-7 w-7 fill-current text-[#5B93F9]"
              ...
              <p class="text-inverted ms-3 text-xs font-medium dark:text-white">
                <span class="inline">{t("add_to_homescreen")}</span>
              </p>
            </div>

            <div class="order-2 flex-shrink-0 sm:order-3">
              <button
                onClick={() => setCloseBanner(true)}
                type="button"
                class="-mr-1 flex rounded-md p-2 hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-white">
                <span class="sr-only">{t("dismiss")}</span>
                <X class="text-inverted h-6 w-6 dark:text-white" aria-hidden="true" />
              </button>
            </div>
          </div>
        </div>
...
function SkeletonLoader() {
	return (
		<ul class="divide-subtle border-subtle bg-default animate-pulse divide-y rounded-md border sm:mx-0 sm:overflow-hidden">
			<SkeletonItem />
			<SkeletonItem />
			<SkeletonItem />
		</ul>
...
function SkeletonItem() {
	return (
		<li>
			<div class="flex items-center justify-between py-5  ltr:pl-4 rtl:pr-4 sm:ltr:pl-0 sm:rtl:pr-0">
				<div class="items-between flex w-full flex-col justify-center sm:px-6">
					<SkeletonText class="my-1 h-4 w-32" />
					<SkeletonText class="my-1 h-2 w-24" />
					<SkeletonText class="my-1 h-2 w-40" />
				</div>
				<Button
					class="mx-5"
					type="button"
...
const CustomEventTypeModal = dynamic(
	() => import("@components/eventtype/CustomEventTypeModal"),
);
...
export const EventAdvancedTab = ({
	...
	const [hashedLinkVisible, setHashedLinkVisible] = useState(
		!!formMethods.getValues("hashedLink"),
	);
	...
	const placeholderHashedLink = `${CAL_URL}/d/${hashedUrl}/${formMethods.getValues(
		"slug",
	)}`;
	const seatsEnabled = formMethods.watch("seatsPerTimeSlotEnabled");
	const noShowFeeEnabled =
		formMethods.getValues("metadata")?.apps?.stripe?.enabled === true &&
		formMethods.getValues("metadata")?.apps?.stripe?.paymentOption ===
			"HOLD";
	...
	const successRedirectUrlLocked =
	...
	return (
		<div class="flex flex-col space-y-4">
			 ...
			 */}
			<div class="border-subtle space-y-6 rounded-lg border p-6">
				{!!connectedCalendarsQuery.data?.connectedCalendars.length &&
					!team && (
						<div class="flex flex-col">
							<div class="flex justify-between">
								<div>
									<Label class="text-emphasis mb-0 font-medium">
										{t("add_to_calendar")}
									</Label>
								</div>
								<Link
									...
									target="_blank"
									class="hover:text-emphasis text-default text-sm">
							...
							<Controller
								...
								render={({ field: { onChange, value } }) => (
									<DestinationCalendarSelector
										value={
											value ? value.externalId : undefined
										}
										onChange={onChange}
										hidePlaceholder={true}
					...
					<TextField
						...
						addOnSuffix={
							<Button
								...
								class="hover:stroke-3 hover:text-emphasis min-w-fit !py-0 px-0 hover:bg-transparent"
								onClick={() =>
			...
			<div class="border-subtle space-y-6 rounded-lg border p-6">
				<FormBuilder
					...
					dataStore={{
						options: {
							locations: getLocationsOptionsForSelect(
			...
			<Controller
				...
				render={({ field: { value, onChange } }) => (
					<SettingsToggle
						...
						description={t(
							"description_requires_booker_email_verification",
						)}
						checked={value}
				...
				render={({ field: { value, onChange } }) => (
					<>
						<SettingsToggle
							...
							<div class="border-subtle rounded-b-lg border border-t-0 p-6">
								<TextField
									class="w-full"
			...
			<SettingsToggle
				...
				Badge={
					<a
						...
						<Info class="ml-1.5 h-4 w-4 cursor-pointer" />
					</a>
				}
					...
					render={({ field: { value, onChange } }) => (
						<>
							<SettingsToggle
								...
								switchContainerClassName="border-subtle rounded-lg border py-6 px-4 sm:px-6"
								title={t(
									"disable_attendees_confirmation_emails",
								)}
								description={t(
		...
		<div class="flex items-end justify-end">
			<div class="w-1/2 md:w-full">
				<InputField
					...
					placeholder="0"
					min={0}
					class="mb-0 h-9 rounded-[4px] ltr:mr-2 rtl:ml-2"
				/>
			...
			<Select
				...
				class="mb-0 ml-2 h-9 w-full capitalize md:min-w-[150px] md:max-w-[200px]"
...
export const EventLimitsTab = () => {
	...
	const [offsetToggle, setOffsetToggle] = useState(
	...
	const offsetAdjustedTime = new Date(
		offsetOriginalTime.getTime() + watchOffsetStartValue * 60 * 1000,
	);
		...
		<div>
			<div class="border-subtle space-y-6 rounded-lg border p-6">
				<div class="flex flex-col space-y-4 lg:flex-row lg:space-x-4 lg:space-y-0">
					...
					<div class="w-full">
						<Label for="afterEventBuffer">{t("after_event")}</Label>
						<Controller
				...
				<div class="flex flex-col space-y-4 lg:flex-row lg:space-x-4 lg:space-y-0">
					<div class="w-full">
						<Label for="minimumBookingNotice">
					...
					<div class="w-full">
						<Label for="slotInterval">{t("slot_interval")}</Label>
						<Controller
							name="slotInterval"
							render={() => {
				...
				render={({ field: { value } }) => {
					...
					return (
						<SettingsToggle
							...
							description={t(
								"limit_booking_frequency_description",
							)}
							checked={isChecked}
				...
				render={({ field: { value } }) => {
					...
					return (
						<SettingsToggle
							...
							description={t(
								"limit_booking_only_first_slot_description",
							)}
							checked={isChecked}
							onCheckedChange={(active) => {
								formMethods.setValue(
				...
				render={({ field: { value } }) => {
					...
					return (
						<SettingsToggle
							...
							description={t(
								"limit_total_booking_duration_description",
							)}
							checked={isChecked}
							onCheckedChange={(active) => {
								if (active) {
										...
										return (
											<div
												...
												<RadioGroup.Item
													id={period.type}
													value={period.type}
													class="min-w-4 bg-default border-default flex h-4 w-4 cursor-pointer items-center rounded-full border focus:border-2 focus:outline-none ltr:mr-2 rtl:ml-2">
													<RadioGroup.Indicator class="after:bg-inverted relative flex h-4 w-4 items-center justify-center after:block after:h-2 after:w-2 after:rounded-full" />
												</RadioGroup.Item>
												...
												{period.type === "ROLLING" && (
													<div class="flex items-center">
														<TextField
															labelSrOnly={true}
															type="number"
															class="border-default my-0 block w-16 text-sm [appearance:textfield] ltr:mr-2 rtl:ml-2"
															placeholder="30"
															{...formMethods.register(
																"periodDays",
																{
												...
												{period.type === "RANGE" && (
													<div class="me-2 ms-2 inline-flex space-x-2 rtl:space-x-reverse">
														<Controller
															name="periodDates"
															render={() => (
																<DateRangePicker
				...
				<div class="border-subtle rounded-b-lg border border-t-0 p-6">
					<TextField
						...
						hint={t("offset_start_description", {
							originalTime: offsetOriginalTime.toLocaleTimeString(
								i18n.language,
								{ timeStyle: "short" },
							),
							adjustedTime: offsetAdjustedTime.toLocaleTimeString(
								i18n.language,
								{ timeStyle: "short" },
							),
						})}
...
type IntervalLimitItemProps = {
	...
	onLimitChange: (
		intervalLimitsKey: IntervalLimitsKey,
		limit: number,
	) => void;
	onIntervalSelect: (
		interval: SingleValue<{ value: keyof IntervalLimit; label: string }>,
	) => void;
};
...
const IntervalLimitItem = ({
	...
	return (
		<div
			...
			class="mb-4 flex max-h-9 items-center space-x-2 text-sm rtl:space-x-reverse"
			key={limitKey}>
			<TextField
				required={true}
				type="number"
				containerClassName={
					textFieldSuffix ? "w-44 -mb-1" : "w-16 mb-0"
				}
				class="mb-0"
				placeholder={`${value}`}
				disabled={disabled}
			...
			<Select
				...
				isSearchable={false}
				isDisabled={disabled}
				defaultValue={INTERVAL_LIMIT_OPTIONS.find(
					(option) => option.value === limitKey,
				)}
						...
						{currentIntervalLimits &&
							...
							Object.entries(currentIntervalLimits)
								.sort(([limitKeyA], [limitKeyB]) => {
									return (
										ascendingLimitKeys.indexOf(
											limitKeyA as IntervalLimitsKey,
										) -
										ascendingLimitKeys.indexOf(
											limitKeyB as IntervalLimitsKey,
										)
									);
								})
								.map(([key, value]) => {
										...
										<IntervalLimitItem
											...
											onIntervalSelect={(interval) => {
												...
												const newData = {
													...
													[interval?.value as IntervalLimitsKey]:
														currentValue,
												};
												onChange(newData);
											}}
	...
	return (
		<SkeletonContainer>
			<div class="mb-4 flex items-center">
				<SkeletonAvatar class="h-8 w-8" />
				<div class="flex flex-col space-y-1">
					<SkeletonText class="h-4 w-16" />
					<SkeletonText class="h-4 w-24" />
				</div>
			</div>
			<ul class="border-subtle bg-default divide-subtle divide-y rounded-md border sm:mx-0 sm:overflow-hidden">
				<SkeletonItem />
				<SkeletonItem />
				<SkeletonItem />
			</ul>
...
function SkeletonItem() {
	return (
		<li class="group flex w-full items-center justify-between px-4 py-4 sm:px-6">
			<div class="flex-grow truncate text-sm">
				<div>
					<SkeletonText class="h-5 w-32" />
				</div>
				<div class="">
					<ul class="mt-2 flex space-x-4 rtl:space-x-reverse ">
						<li class="flex items-center whitespace-nowrap">
							<Clock class="text-subtle mr-1.5 mt-0.5 inline h-4 w-4" />
							<SkeletonText class="h-4 w-12" />
						</li>
						<li class="flex items-center whitespace-nowrap">
							<User class="text-subtle mr-1.5 mt-0.5 inline h-4 w-4" />
							<SkeletonText class="h-4 w-12" />
						</li>
					</ul>
				</div>
...
import { Lock, Plus } from "@calcom/ui/components/icon";
...
export const EventWebhooksTab = ({
	...
	const { data: installedApps, isLoading } =
		trpc.viewer.integrations.useQuery({
			variant: "other",
			onlyInstalled: true,
		});
							...
							<>
								{isManagedEventType && (
									<Alert
										...
										message={t(
											"locked_webhooks_description",
										)}
									/>
								)}
								{webhooks.length ? (
									<>
										<div class="border-subtle mb-2 rounded-md border p-8">
											<div class="text-default text-sm font-semibold">
												{t("webhooks")}
											</div>
											<p class="text-subtle max-w-[280px] break-words text-sm sm:max-w-[500px]">
												{t("add_webhook_description", {
													appName: APP_NAME,
												})}
											</p>

											<div class="border-subtle my-8 rounded-md border">
												{webhooks.map(
													(webhook, index) => {
														return (
															<WebhookListItem
											...
											<p class="text-default text-sm font-normal">
												<Trans i18nKey="edit_or_manage_webhooks">
													If you wish to edit or
													manage your web hooks,
													please head over to &nbsp;
													<Link
							...
							<WebhookForm
								...
								onSubmit={(values: WebhookFormSubmitData) => {
									if (
										...
										showToast(
											t(
												"webhook_subscriber_url_reserved",
											),
											"error",
										);
										return;
									...
									if (values.changeSecret) {
										values.secret = values.newSecret.length
											? values.newSecret
											: null;
									}
  ...
  return (
    <Dialog open={slugExistsChildrenDialogOpen.length > 0} onOpenChange={onOpenChange}>
      <ConfirmationDialogContent
        ...
        <p class="mt-5">{t("managed_event_dialog_clarification")}</p>
      </ConfirmationDialogContent>
    </Dialog>
  );
}
...
export default function InstantEventController({
  ...
  return (
    <LicenseRequired>
      <div class="block items-start sm:flex">
        {!isOrg || !isTeamEvent ? (
          <EmptyScreen
          ...
          <div class={paymentEnabled ? "" : "w-full"}>
            {paymentEnabled ? (
...
const InstantMeetingWebhooks = ({
	...
	const { data: installedApps, isPending } =
		trpc.viewer.integrations.useQuery({
			variant: "other",
			onlyInstalled: true,
		});
			...
			{webhooks && !isPending && (
				<>
					<div>
						{webhooks.length ? (
							<>
								<div class="border-subtle my-2 rounded-md border">
							...
							<>
								<p class="text-default mb-4 text-sm font-normal">
									{t("warning_payment_instant_meeting_event")}
								</p>
								<EmptyScreen
									Icon={TbWebhook}
									...
									description={t(
										"create_instant_meeting_webhook_description",
									)}
									buttonRaw={
										isChildrenManagedEventType &&
										!isManagedEventType ? (
...
import type {
	GetAppData,
	SetAppData,
} from "@calcom/app-store/EventTypeAppContext";
...
export const EventAppsTab = ({ eventType }: { eventType: EventType }) => {
	const { t } = useLocale();
	const { data: eventTypeApps, isPending } =
		trpc.viewer.integrations.useQuery({
	...
	const installedApps =
		eventTypeApps?.items.filter(
			(app) => app.userCredentialIds.length || app.teams.length,
		) || [];
	...
	const getAppDataGetter = (appId: EventTypeAppsList): GetAppData => {
		return (key) => {
			const appData =
				allAppsData[appId as keyof typeof allAppsData] || {};
			if (key) {
	...
	const getAppDataSetter = (
		...
		return (key, value) => {
			...
			const allAppsDataFromForm =
				formMethods.getValues("metadata")?.apps || {};
	...
	const appsWithTeamCredentials =
		eventTypeApps?.items.filter((app) => app.teams.length) || [];
		...
		<>
			<div>
				<div class="before:border-0">
					...
					{cardsForAppsWithTeams.map((apps) =>
						apps.map((cards) => cards),
					)}
			...
			{!shouldLockDisableProps("apps").disabled && (
				<div class="bg-muted mt-6 rounded-md p-8">
					{!isPending && notInstalledApps?.length ? (
						<>
							<h2 class="text-emphasis mb-2 text-xl font-semibold leading-5 tracking-[0.01em]">
								{t("available_apps_lower_case")}
							</h2>
							<p class="text-default mb-6 text-sm font-normal">
								<Trans i18nKey="available_apps_desc">
									View popular apps below and explore more in
									our &nbsp;
									<Link
					...
					<div class="bg-default border-subtle divide-subtle divide-y rounded-md border before:border-0">
...
import { classNames } from "@calcom/lib";
import { useLocale } from "@calcom/lib/hooks/useLocale";
import { Input, RadioField, Select, SettingsToggle } from "@calcom/ui";
            ...
            <SettingsToggle
              ...
              <div class="border-subtle rounded-b-lg border border-t-0 p-6">
                <RadioGroup.Root
                  ...
                  <div class="flex flex-col flex-wrap justify-start gap-y-2">
                    {(requiresConfirmationSetup === undefined ||
                                ...
                                time: (
                                  <div class="mx-2 inline-flex">
                                    <Input
                                      ...
                                      class="border-default !m-0 block w-16 rounded-r-none border-r-0 text-sm [appearance:textfield] focus:z-10 focus:border-r"
...
const CustomEventTypeModalForm: FC<CustomEventTypeModalFormProps> = (props) => {
	...
	const placeHolder_ =
		watch("customEventName") === "" ? previewText : placeHolder;
	...
	return (
		<form
				...
				{...register("customEventName", {
					validate: (value) =>
						validateCustomEventName(
							value,
							t("invalid_event_name_variables"),
							event.bookingFields,
						),
				})}
				class="mb-0"
			...
			<div class="pt-6 text-sm">
				<div class="bg-subtle mb-6 rounded-md p-2">
					<h1 class="text-emphasis mb-2 ml-1 font-medium">
						{t("available_variables")}
					</h1>
					<div class="mb-2.5 flex font-normal">
						<p class="text-subtle ml-1 mr-5 w-28">{`{Event type title}`}</p>
						<p class="text-emphasis">{t("event_name_info")}</p>
					</div>
					<div class="mb-2.5 flex font-normal">
						<p class="text-subtle ml-1 mr-5 w-28">{`{Organiser}`}</p>
						<p class="text-emphasis">{t("your_full_name")}</p>
					</div>
					<div class="mb-2.5 flex font-normal">
						<p class="text-subtle ml-1 mr-5 w-28">{`{Scheduler}`}</p>
						<p class="text-emphasis">{t("scheduler_full_name")}</p>
					</div>
					<div class="mb-1 flex font-normal">
						<p class="text-subtle ml-1 mr-5 w-28">{`{Location}`}</p>
						<p class="text-emphasis">{t("location_info")}</p>
					</div>
				</div>
				<h1 class="mb-2 text-[14px] font-medium leading-4">
					{t("preview")}
				</h1>
				<div
					class="flex h-[212px] w-full rounded-md border-y bg-cover bg-center dark:invert"
					style={{
						backgroundImage: "url(/calendar-preview.svg)",
					}}>
					<div class="m-auto flex items-center justify-center self-stretch">
						<div class="bg-subtle ml-11 mt-3 box-border h-[110px] w-[120px] flex-col items-start gap-1 rounded-md border border-solid border-black text-[12px] leading-3">
							<p class="text-emphasis overflow-hidden text-ellipsis p-1.5 font-medium">
								{previewText}
							</p>
							<p class="text-default ml-1.5 text-[10px] font-normal">
								8 - 10 AM
							</p>
						</div>
					</div>
        ...
        {paymentEnabled ? (
          ...
          <>
            ...
            <SettingsToggle
              ...
              <div class="border-subtle rounded-b-lg border border-t-0 p-6">
                {recurringEventState && (
                  <div data-testid="recurring-event-collapsible" class="text-sm">
                    <div class="flex items-center">
                      <p class="text-emphasis ltr:mr-2 rtl:ml-2">{t("repeats_every")}</p>
                      <TextField
                      ...
                      <Select
                        ...
                        isSearchable={false}
                        class="w-18 ml-2 block min-w-0 rounded-md text-sm"
                        isDisabled={recurringLocked.disabled}
                        onChange={(event) => {
                          const newVal = {
                            ...recurringEventState,
                            freq: Number.parseInt(event?.value || `${Frequency.WEEKLY}`),
                          };
                          formMethods.setValue("recurringEvent", newVal);
                          setRecurringEventState(newVal);
                        }}
                    ...
                    <div class="mt-4 flex items-center">
                      <p class="text-emphasis ltr:mr-2 rtl:ml-2">{t("for_a_maximum_of")}</p>
                      <TextField
                        disabled={recurringLocked.disabled}
                        type="number"
                        min="1"
                      ...
                      />
                      <p class="text-emphasis ltr:ml-2 rtl:mr-2">
                        {t("events", {
...
import { TRPCClientError, trpc } from "@calcom/trpc/react";
import {
	Button,
	ButtonGroup,
	ConfirmationDialogContent,
...
function getNavigation({
	...
	return [
		...
		},
		{
			...
			info: `${installedAppsNumber} apps, ${enabledAppsNumber} ${t(
				"active",
			)}`,
		},
		{
...
function EventTypeSingleLayout({
	...
	const EventTypeTabs = useMemo(() => {
		...
		navigation.splice(1, 0, {
			name: "availability",
			href: `/event-types/${formMethods.getValues(
				"id",
		...
		<Shell
				...
				<div class="flex items-center justify-end">
					{!formMethods.getValues("metadata")?.managedEventConfig && (
						<>
							<div
								...
								{formMethods.watch("hidden") && (
									<Skeleton
										as={Label}
										for="hiddenSwitch"
										class="mt-2 hidden cursor-pointer self-center whitespace-nowrap pr-2 sm:inline">
										{t("hidden")}
									</Skeleton>
								)}
								<Tooltip
									...
									side="bottom">
									<div class="self-center rounded-md p-2">
										<Switch
											id="hiddenSwitch"
											checked={
												!formMethods.watch("hidden")
							...
							<VerticalDivider class="hidden lg:block" />
						</>
					)}
					...
					<ButtonGroup
						combined={true}
						containerProps={{
							className: "border-default hidden lg:flex",
						}}>
						{!isManagedEventType && (
							<>
					...
					<Dropdown>
						...
						<DropdownMenuContent style={{ minWidth: "200px" }}>
							<DropdownMenuItem class="focus:ring-muted">
								<DropdownItem
									target="_blank"
									type="button"
									StartIcon={ExternalLink}
							...
							<DropdownMenuSeparator />
							<div class="hover:bg-subtle flex h-9 cursor-pointer flex-row items-center justify-between px-4 py-2">
								<Skeleton
									as={Label}
									for="hiddenSwitch"
									class="mt-2 inline cursor-pointer self-center pr-2 ">
									{formMethods.watch("hidden")
										? t("show_eventtype_on_profile")
										: t("hide_from_profile")}
								</Skeleton>
					...
					<div class="border-default border-l-2" />
					<Button
						class="ml-4 lg:ml-0"
						type="submit"
			...
			<Suspense fallback={<Loader />}>
				<div class="flex flex-col xl:flex-row xl:space-x-6">
					...
					<div class="p-2 md:mx-0 md:p-0 xl:hidden">
						<HorizontalTabs
							tabs={EventTypeTabs}
							linkShallow={true}
						/>
					</div>
					<div class="w-full ltr:mr-2 rtl:ml-2">
						<div
							class={classNames(
								"bg-default border-subtle  mt-4 rounded-md sm:mx-0 xl:mt-0",
								disableBorder
									? "border-0 "
									: "p-2 md:border md:p-6",
							)}>
							{children}
						</div>
					</div>
				...
				<ConfirmationDialogContent
					...
					<p class="mt-5">
						<Trans
							...
							components={{
								li: <li />,
								ul: <ul class="ml-4 list-disc" />,
							}}>
							<ul>
								<li>
									Members assigned to this event type will
									also have their event types deleted.
								</li>
								<li>
									Anyone who they&apos;ve shared their link
									with will no longer be able to book using
									it.
								</li>
							</ul>
						</Trans>
...
const EventTypeScheduleDetails = memo(
	({
		...
		const { isPending, data: schedule } =
			trpc.viewer.availability.schedule.get.useQuery(
				{
					scheduleId:
						scheduleId ||
						loggedInUser?.defaultScheduleId ||
		...
		const filterDays = (dayNum: number) =>
			schedule?.schedule.filter((item) =>
				item.days.includes((dayNum + 1) % 7),
			) || [];

		return (
			<div>
				<div class="border-subtle space-y-4 border-x p-6">
					<ol class="table border-collapse text-sm">
						{weekdayNames(i18n.language, 1, "long").map(
							(day, index) => {
								const isAvailable = !!filterDays(index).length;
								return (
									<li
										key={day}
										class="my-6 flex border-transparent last:mb-2">
										<span
											class={classNames(
										...
										{isPending ? (
											<SkeletonText class="block h-5 w-60" />
										) : isAvailable ? (
											<div class="space-y-3 text-right">
												{filterDays(index).map(
													(dayRange, i) => (
														<div
															key={i}
															class="text-default flex items-center leading-4">
															<span class="w-16 sm:w-28 sm:text-left">
																{format(
																	dayRange.startTime,
																	timeFormat ===
																		12,
											...
											<span class="text-subtle ml-6 sm:ml-0">
												{t("unavailable")}
											</span>
										)}
									</li>
				...
				<div class="bg-muted border-subtle flex flex-col justify-center gap-2 rounded-b-md border p-6 sm:flex-row sm:justify-between">
					<span class="text-default flex items-center justify-center text-sm sm:justify-start">
						<Globe class="h-3.5 w-3.5 ltr:mr-2 rtl:ml-2" />
						{schedule?.timeZone || (
							<SkeletonText class="block h-5 w-32" />
						)}
					</span>
					{!!schedule?.id &&
						!schedule.isManaged &&
...
const EventTypeSchedule = ({ eventType }: { eventType: EventTypeSetup }) => {
	...
	useEffect(
		function refactorMeWithoutEffect() {
			...
			// We push the selected schedule from the event type if it's not part of the list response. This happens if the user is an admin but not the schedule owner.
				...
				eventType.schedule &&
				!schedules.find(
					(schedule) => schedule.id === eventType.schedule,
				)
			) {
				options.push({
	...
	return (
		<div>
			<div class="border-subtle rounded-t-md border p-6">
				<label
					for="availability"
					class="text-default mb-2 block text-sm font-medium leading-none">
					{t("availability")}
						...
						render={({ field }) => {
							return (
								<Select
									...
									class="block w-full min-w-0 flex-1 rounded-sm text-sm"
			...
			{availabilityValue?.value !== 0 ? (
				...
				isManagedEventType && (
					<p class="!mt-2 ml-1 text-sm text-gray-600">
...
const sortByLabel = (
	a: ReturnType<typeof mapUserToValue>,
	b: ReturnType<typeof mapUserToValue>,
) => {
	if (a.label < b.label) {
		return -1;
	}
...
const ChildrenEventTypesList = ({
	...
	onChange?: (
		options: ReturnType<typeof mapMemberToChildrenOption>[],
	) => void;
			...
			<div>
				...
				<ChildrenEventTypeSelect
					...
					options={options.filter(
						(opt) =>
							!value.find(
								(val) => val.owner.id.toString() === opt.value,
							),
					)}
...
>) => {
	return (
		<div class="bg-muted flex flex-col space-y-5 p-4">
			<div>
				{labelText ? <Label>{labelText}</Label> : <></>}
				<CheckedTeamSelect
					isOptionDisabled={(option) =>
						!!value.find(
							(host) => host.userId.toString() === option.value,
						)
					}
					onChange={(options) => {
						onChange &&
					...
					value={(value || [])
						...
						.map(
							(host) =>
								...
								options.find(
									(member) =>
										member.value === host.userId.toString(),
								)!,
						)
						.filter(Boolean)}
					controlShouldRenderValue={false}
					...
					{...rest}
				/>
				{helperText && <p class="text-subtle text-sm">{helperText}</p>}
			</div>
		</div>
	);
};
...
const RoundRobinHosts = ({
	...
	return (
		<div class="border-subtle mt-6 space-y-5 rounded-lg border px-4 py-6 sm:px-6">
			<div class="flex flex-col gap-8">
				...
				<div class="bg-muted flex flex-col">
					<Label class="p-4">{t("round_robin_hosts")}</Label>
					<div class="px-4 pb-2">
						<AssignAllTeamMembers
	...
	return (
        ...
        addOnSuffix={<>{t("hosts")}</>}
				...
				return !!schedulingType ? (
					schedulingTypeRender[schedulingType]
				) : (
					<></>
				);
			}}
...
import { Check, CornerDownRight, Plus, X } from "@calcom/ui/components/icon";
...
const DescriptionEditor = ({ isEditable }: { isEditable: boolean }) => {
	...
	return mounted ? (
		...
		<SkeletonContainer>
			<SkeletonText class="block h-24 w-full" />
		</SkeletonContainer>
	);
};
...
export const EventSetupTab = (
	...
	const locationOptions = props.locationOptions.map((locationOption) => {
		const options = locationOption.options.filter((option) => {
			...
			return team
				? true
				: option.label !== t("organizer_default_conferencing_app");
		});
	...
	const Locations = () => {
		...
		const validLocations = formMethods
			.getValues("locations")
			.filter((location) => {
		...
		const defaultValue = isManagedEventType
			? locationOptions.find((op) => op.label === t("default"))
					?.options[0]
			: undefined;
		...
		const [showEmptyLocationSelect, setShowEmptyLocationSelect] =
			useState(false);
		const [selectedNewOption, setSelectedNewOption] =
			useState<SingleValueLocationOption | null>(null);

		return (
			<div class="w-full">
				<ul ref={animationRef} class="space-y-2">
					{locationFields.map((field, index) => {
						const eventLocationType = getEventLocationType(
							field.type,
						);
							...
							<li key={field.id}>
								<div class="flex w-full items-center">
									<LocationSelect
										...
										isDisabled={
											shouldLockDisableProps("locations")
												.disabled
										}
										defaultValue={option}
										isSearchable={false}
										class="block min-w-0 flex-1 rounded-sm text-sm"
										menuPlacement="auto"
										onChange={(
											e: SingleValueLocationOption,
										) => {
											if (e?.value) {
									...
									<button
										...
										class="min-h-9 block h-9 px-2"
										type="button"
										onClick={() => remove(index)}
										aria-label={t("remove")}>
										<div class="h-4 w-4">
											<X class="border-l-1 hover:text-emphasis text-subtle h-4 w-4" />
										</div>
									</button>
								</div>
									...
									<div class="mt-2 space-y-2">
										<div class="w-full">
											...
											<ErrorMessage
												...
												class="text-error my-1 ml-6 text-sm"
												as="div"
												id="location-error"
											/>
										</div>
					...
					{validLocations.some(
						(location) =>
							location.type === MeetLocationType &&
							destinationCalendar?.integration !==
								"google_calendar",
					) && (
						<div class="text-default flex items-center text-sm">
							<div class="mr-1.5 h-3 w-3">
								<Check class="h-3 w-3" />
							</div>
							<Trans i18nKey="event_type_requres_google_cal">
								<p>
									The “Add to calendar” for this event type
									needs to be a Google Calendar for Meet to
									work. Change it{" "}
									<Link
										href={`${CAL_URL}/event-types/${formMethods.getValues(
											"id",
										)}?tabName=advanced`}
										class="underline">
										here.
					...
					{isChildrenManagedEventType &&
						!locationAvailable &&
						locationDetails && (
							<p class="pl-1 text-sm leading-none text-red-600">
								{t("app_not_connected", {
									appName: locationDetails.name,
								})}{" "}
								<a
									class="underline"
									href={`${CAL_URL}/apps/${locationDetails.slug}`}>
									{t("connect_now")}
								</a>
							</p>
						)}
			...
			<div class="space-y-4">
				<div class="border-subtle space-y-6 rounded-lg border p-6">
					...
					<div>
						...
						<DescriptionEditor
							isEditable={!descriptionLockedProps.disabled}
						/>
					</div>
					<TextField
						required={true}
				...
				<div class="border-subtle rounded-lg border p-6">
						...
						<div class="space-y-6">
							<div>
								...
								<Select
									...
									isSearchable={false}
									class="h-auto !min-h-[36px] text-sm"
					...
					{!lengthLockedProps.disabled && (
						<div class="!mt-4 [&_label]:my-1 [&_label]:font-normal">
							<SettingsToggle
								...
								tooltip={
									seatsEnabled
										? t(
												"seat_options_doesnt_multiple_durations",
											)
										: undefined
								}
								onCheckedChange={() => {
...
const ErrorDebugPanel: React.FC<{ error: Props["error"]; children?: never }> = (
	...
	return (
		<div class="bg-default overflow-hidden shadow sm:rounded-lg">
			<div class="border-subtle border-t px-4 py-5 sm:p-0">
				<dl class="sm:divide-subtle sm:divide-y">
						...
						if (value !== undefined) {
							return (
								<div
									...
									<dt class="text-emphasis text-sm font-bold">
										{key}
									</dt>
									<dd class="text-emphasis mt-1 text-sm sm:col-span-2 sm:mt-0">
										{value}
									</dd>
								</div>
							);
...
export const ErrorPage: React.FC<Props> = (props) => {
	...
	return (
		<>
			<div class="bg-default min-h-screen px-4">
				<main class="mx-auto max-w-xl pb-6 pt-16 sm:pt-24">
					<div class="text-center">
						<p class="text-emphasis text-sm font-semibold uppercase tracking-wide">
							{statusCode}
						</p>
						<h1 class="text-emphasis mt-2 text-4xl font-extrabold tracking-tight sm:text-5xl">
							{message}
						</h1>
					</div>
				</main>
...
const ShellHeading = () => {
	const { t } = useLocale();
	return <span class="block py-2">{t("app_store")}</span>;
};

export default function WrappedApp(props: AppPageProps) {
  return (
    <Shell smallHeading={true} isPublic={true} hideHeadingOnMobile={true} heading={<ShellHeading />} backPath="/apps" withoutSeo={true}>
      <HeadSeo
        title={props.name}
...
export const InstallAppButtonChild = ({
	...
	const shouldDisableInstallation = multiInstall
		? false
		...
		<Dropdown>
			...
			<DropdownMenuPortal>
				<DropdownMenuContent
					...
					{mutation.isPending && (
						<div class="z-1 fixed inset-0 flex items-center justify-center">
							<Spinner />
						</div>
					)}
					...
					{userAdminTeams.map((team) => {
						const isInstalled =
							...
							credentials.some((credential) =>
								credential?.teamId
									? credential?.teamId === team.id
									: credential.userId === team.id,
							);
...
import {
	...
	InstallAppButton,
} from "@calcom/app-store/components";
...
export const AppPage = ({
	...
	const [showDisconnectIntegration, setShowDisconnectIntegration] =
		useState(false);
	...
	return (
		<div class="relative flex-1 flex-col items-start justify-start px-4 md:flex md:px-8 lg:flex-row lg:px-0">
			{hasDescriptionItems && (
				<div class="align-center bg-subtle -ml-4 -mr-4 mb-4 flex min-h-[450px] w-auto basis-3/5 snap-x snap-mandatory flex-row overflow-auto whitespace-nowrap p-4  md:-ml-8 md:-mr-8 md:mb-8 md:p-8 lg:mx-0 lg:mb-0 lg:max-w-2xl lg:flex-col lg:justify-center lg:rounded-md">
					{descriptionItems ? (
						descriptionItems.map((descriptionItem, index) =>
							typeof descriptionItem === "object" ? (
								<div
									key={`iframe-${index}`}
									class="mr-4 max-h-full min-h-[315px] min-w-[90%] max-w-full snap-center last:mb-0 lg:mb-4 lg:mr-0 [&_iframe]:h-full [&_iframe]:min-h-[315px] [&_iframe]:w-full">
									<iframe
										allowFullScreen={true}
										{...descriptionItem.iframe}
									/>
								</div>
							) : (
								<img
									...
									class="mr-4 h-auto max-h-80 max-w-[90%] snap-center rounded-md object-contain last:mb-0 md:max-h-min lg:mb-4 lg:mr-0  lg:max-w-full"
								/>
							),
						)
					) : (
			...
			<div
					...
					<header>
						<div class="mb-4 flex items-center">
							<img
								...
								alt={name}
							/>
							<h1 class="font-cal text-emphasis ml-4 text-3xl">
								{name}
							</h1>
						</div>
						<h2 class="text-default text-sm font-medium">
							<Link
								href={`categories/${categories[0]}`}
								class="bg-subtle text-emphasis rounded-md p-1 text-xs capitalize">
								{categories[0]}
							</Link>{" "}
							{paid && (
								<>
						...
						{isTemplate && (
							<Badge variant="red" class="mt-4">
								Template - Available in Dev Environment only for
								testing
							</Badge>
						)}
					</header>
				...
				{appDbQuery.isPending ? (
					<SkeletonButton class="h-10 w-24" />
				) : (
					isGlobal ||
							...
							render={({ useDefaultComponent, ...props }) => {
								...
								return (
									<InstallAppButtonChild
										appCategories={categories}
										userAdminTeams={
											appDbQuery.data?.userAdminTeams
										}
										addAppMutationInput={{
											type,
											variant,
				...
				{dependencies &&
					(dependencyData.isPending ? (
						<SkeletonButton class="mt-6 h-20 grow" />
					) : (
						<div class="mt-6">
							<AppDependencyComponent
								appName={name}
				...
				<div class="prose-sm prose prose-a:text-default prose-headings:text-emphasis prose-code:text-default prose-strong:text-default text-default mt-8">
					{body}
				</div>
				{!paid && (
					<>
						<h4 class="text-emphasis mt-8 font-semibold ">
							{t("pricing")}
						</h4>
				...
				<h4 class="text-emphasis mb-2 mt-8 font-semibold ">
					{t("contact")}
				</h4>
				<ul class="prose-sm -ml-1 -mr-1 leading-5">
					{docs && (
						<li>
							<a
								target="_blank"
								rel="noreferrer"
								class="text-emphasis text-sm font-normal no-underline hover:underline"
								href={docs}>
								<BookOpen class="text-subtle -mt-1 mr-1 inline h-4 w-4" />
								{t("documentation")}
							</a>
						</li>
					)}
					{website && (
						<li>
							<a
								target="_blank"
								rel="noreferrer"
								class="text-emphasis font-normal no-underline hover:underline"
								href={website}>
								<ExternalLink class="text-subtle -mt-px mr-1 inline h-4 w-4" />
					...
					{email && (
						<li>
							<a
								...
								class="text-emphasis font-normal no-underline hover:underline"
								href={`mailto:${email}`}>
								<Mail class="text-subtle -mt-px mr-1 inline h-4 w-4" />
					...
					{tos && (
						<li>
							<a
								...
								class="text-emphasis font-normal no-underline hover:underline"
								href={tos}>
								<File class="text-subtle -mt-px mr-1 inline h-4 w-4" />
								{t("terms_of_service")}
							</a>
						</li>
					)}
					{privacy && (
						<li>
							<a
								...
								class="text-emphasis font-normal no-underline hover:underline"
								href={privacy}>
								<Shield class="text-subtle -mt-px mr-1 inline h-4 w-4" />
								{t("privacy_policy")}
							</a>
						</li>
					)}
				</ul>
				<hr class="border-subtle my-8 border" />
				<span class="leading-1 text-subtle block text-xs">
				...
				<a
					...
					<Flag class="inline h-3 w-3" /> {t("report_app")}
				</a>
			</div>
		</div>
	);
	...
	return (
		<QueryCell
			...
			success={({ data }) => {
				...
				return (
					<div class="border-subtle mt-6 rounded-lg border">
						<div class="border-subtle border-b p-6">
							<div class="flex items-center justify-between">
								<div>
									<h4 class="text-emphasis text-base font-semibold leading-5">
						...
						<List noBorderTreatment={true} class="p-6 pt-2">
								...
								<Fragment key={item.credentialId}>
									{item.calendars ? (
										<AppListCard
											...
											class="border-subtle mt-4 rounded-lg border"
											actions={
											...
											<div class="border-subtle border-t">
												{!fromOnboarding && (
													<>
														<p class="text-subtle px-5 pt-4 text-sm">
															{t(
																"toggle_calendars_conflict",
															)}
														</p>
										...
										<Alert
											...
											message={
												<span>
													<Link
														href={`/apps/${item.integration.slug}`}>
											...
											actions={
												<div class="flex w-32 justify-end md:pr-1">
													<DisconnectIntegration
														credentialId={
															item.credentialId
														}
		...
		<QueryCell
			...
			success={({ data }) => {
				return (
					<>
						{!!data.connectedCalendars.length ||
						!!installedCalendars.data?.items.length ? (
							<>
								{heading && (
									<>
										<div class="border-subtle mb-6 mt-8 rounded-lg border">
											<div class="p-6">
												<h2 class="text-emphasis mb-1 text-base font-bold leading-5 tracking-wide">
													{t("add_to_calendar")}
												</h2>

												<p class="text-subtle text-sm leading-tight">
													{t(
											...
											<div class="border-t">
												<div class="border-subtle flex w-full flex-col space-y-3 border-y-0 p-6">
													<div>
														<Label class="text-default mb-0 font-medium">
															{t("add_events_to")}
														</Label>
														<DestinationCalendarSelector
															hidePlaceholder={
							...
							<EmptyScreen
								...
								description={t(
									`no_category_apps_description_calendar`,
								)}
								buttonRaw={
									<Button
										color="secondary"
...
export default function AppsLayout({ children, actions, emptyStore, ...rest }: AppsLayoutProps) {
  ...
  if (session.status === "loading") return <></>;
  return (
    <ShellMain {...rest} actions={actions?.("block")} hideHeadingOnMobile={true}>
...
export default function InstalledAppsLayout({
  ...
  return (
    <Shell {...rest} hideHeadingOnMobile={true}>
...
export const AppList = ({
							...
							<Dropdown modal={false}>
								...
								<DropdownMenuContent>
									...
									<ConnectOrDisconnectIntegrationMenuItem
										credentialId={
											item.credentialOwner
												?.credentialId ||
											item.userCredentialIds[0]
										}
										type={item.type}
	...
	const appsWithTeamCredentials = data.items.filter(
		(app) => app.teams.length,
	);
	const cardsForAppsWithTeams = appsWithTeamCredentials.map((app) => {
		const appCards = [];
	...
	return (
		<>
			...
			{bulkUpdateModal && (
				<BulkEditDefaultConferencingModal
...
import { Avatar, Badge, ListItemText } from "@calcom/ui";
import { AlertCircle } from "@calcom/ui/components/icon";
...
export default function AppListCard(props: AppListCardProps) {
  ...
  return (
    <div class={classNames(highlight && "dark:bg-muted bg-yellow-100", className)}>
      <div class="flex items-center gap-x-3 px-4 py-4 sm:px-6">
        {logo ? (
          <img
            class={classNames(logo.includes("-dark") && "dark:invert", "h-10 w-10")}
            src={logo}
            alt={`${title} logo`}
          />
        ) : null}
        <div class="flex grow flex-col gap-y-1 truncate">
          <div class="flex items-center gap-x-2">
            <h3 class="text-emphasis truncate text-sm font-semibold">{title}</h3>
            <div class="flex items-center gap-x-2">
          ...
          {invalidCredential && (
            <div class="flex gap-x-2 pt-2">
              <AlertCircle class="h-8 w-8 text-red-500 sm:h-4 sm:w-4" />
              <ListItemText component="p" class="whitespace-pre-wrap text-red-500">
      ...
      {children && <div class="w-full">{children}</div>}
    </div>
  );
}
("use client");
...
interface QueryCellOptionsBase<TData, TError extends ErrorLike> {
	...
	loading?: (
		query: QueryObserverPendingResult<TData, TError> | null,
	) => JSXElementOrNull;
}
...
export function QueryCell<TData, TError extends ErrorLike>(
	...
	if (!isLocaleReady) {
		return (
			opts.loading?.(query.status === "pending" ? query : null) ??
			StatusLoader
		);
	}
...
const CalcomThemeProvider = (props: CalcomThemeProps) => {
	...
	return (
		<ThemeProvider
			{...getThemeProviderProps({ props, isEmbedMode, embedNamespace })}>
...
function getThemeProviderProps({
	...
	const forcedTheme =
		themeSupport === ThemeSupport.None ? "light" : undefined;
...
export const getServerSideProps = async (ctx: GetServerSidePropsContext) => {
	const {
		user: teamOrUserSlugOrDynamicGroup,
		orgSlug,
		type,
	} = paramsSchema.parse(ctx.params);
...
export const getServerSideProps = async (ctx: GetServerSidePropsContext) => {
	...
	return GSSUserPage({
		...ctx,
		query: { user: ctx.query.user, redirect: ctx.query.redirect },
	});
};
import type { GetServerSidePropsContext } from "next";
import { z } from "zod";
...
export const getServerSideProps = async (ctx: GetServerSidePropsContext) => {
	...
	const { username: preFillusername, email: prefilEmail } = querySchema.parse(
		ctx.query,
	);

	if (
		(process.env.NEXT_PUBLIC_DISABLE_SIGNUP === "true" && !token) ||
		flags["disable-signup"]
	) {
		return {
			notFound: true,
		} as const;
...
export function asNumberOrUndefined(str: unknown) {
	return typeof str === "string" ? Number.parseInt(str) : undefined;
}

/** @deprecated use zod instead  */
export function asNumberOrThrow(str: unknown) {
	return Number.parseInt(asStringOrThrow(str));
}
...
export const getServerSideProps = async (
	...
	const { slug: teamSlug, type: meetingSlug } = paramsSchema.parse(
		context.params,
	);
	const {
		rescheduleUid,
	...
	if (rescheduleUid) {
		booking = await getBookingForReschedule(
			`${rescheduleUid}`,
			session?.user?.id,
		);
	}
	...
	return {
		props: {
			...
			isInstantMeeting:
				eventData.isInstantEvent && queryIsInstantMeeting
					? true
					: false,
			themeBasis: null,
		},
...
export const getServerSideProps = async (
	...
	const slug =
		getTheLastArrayElement(context.query.slug) ??
		getTheLastArrayElement(context.query.orgSlug);
...
export const getServerSideProps = async (
	...
	const token = Array.isArray(context.query?.token)
...
const AppWithTranslationHoc = appWithTranslation(({ children }) => (
	<>{children}</>
));

const CustomI18nextProvider = (props: {
...
const CalcomThemeProvider = (props: CalcomThemeProps) => {
	...
	const searchParams = useSearchParams();
	const embedNamespace = searchParams
		? getEmbedNamespace(searchParams)
		: null;
	const isEmbedMode = typeof embedNamespace === "string";
					...
					<CustomI18nextProvider i18n={props.i18n}>
						<TooltipProvider>
							...
							<CalcomThemeProvider
								...
								nonce={props.nonce}
								isThemeSupported={
									/* undefined gets treated as true */ props.isThemeSupported ??
									true
								}
								isBookingPage={
...
} from "@calcom/app-store/_appRegistry";
import { getServerSession } from "@calcom/features/auth/lib/getServerSession";
...
export const getServerSideProps = async (
	...
	if (session?.user?.id) {
		userAdminTeams = await getUserAdminTeams({
			userId: session.user.id,
			getUserInfo: true,
		});
		appStore = await getAppRegistryWithCredentials(
			session.user.id,
			userAdminTeams,
		);
	} else {
...
export function isBrandingHidden(
	hideBrandingSetting: boolean,
	hasPaidPlan: boolean,
) {
...
type BookingListingStatus = z.infer<
	NonNullable<typeof filterQuerySchema>
>["status"];
...
export default function Bookings() {
  ...
  return (
    <ShellMain hideHeadingOnMobile={true} heading={t("bookings")} subtitle={t("bookings_description")}>
      <div class="flex flex-col">
        ...
        <main class="w-full">
          <div class="flex w-full flex-col" ref={animationParentRef}>
            {query.status === "error" && (
            ...
            {query.status === "success" && !isEmpty && (
              <>
                {!!bookingsToday.length && status === "upcoming" && (
                  <div class="mb-6 pt-2 xl:pt-0">
                    <WipeMyCalActionButton bookingStatus={status} bookingsEmpty={isEmpty} />
                    <p class="text-subtle mb-2 text-xs font-medium uppercase leading-4">{t("today")}</p>
                    <div class="border-subtle overflow-hidden rounded-md border">
                      <table class="w-full max-w-full table-fixed">
                        <tbody class="bg-default divide-subtle divide-y" data-testid="today-bookings">
                          <Fragment>
                ...
                <div class="pt-2 xl:pt-0">
                  <div class="border-subtle overflow-hidden rounded-md border">
                    <table data-testid={`${status}-bookings`} class="w-full max-w-full table-fixed">
                      <tbody class="bg-default divide-subtle divide-y" data-testid="bookings">
                  ...
                  <div class="text-default p-4 text-center" ref={buttonInView.ref}>
                    <Button
                      color="minimal"
            ...
            {query.status === "success" && isEmpty && (
              <div class="flex items-center justify-center pt-2 xl:pt-0">
                <EmptyScreen
                  Icon={Calendar}
...
export default function Success(props: PageProps) {
  ...
  function setIsCancellationMode(value: boolean) {
    ...
    } else if (_searchParams.get("cancel")) {
  ...
  return (
    <div class={isEmbed ? "" : "h-screen"} data-testid="success-page">
      {!isEmbed && (
      ...
      {isLoggedIn && !isEmbed && (
        <div class="-mb-4 ml-4 mt-2">
          <Link
            ...
            class="hover:bg-subtle text-subtle hover:text-default mt-2 inline-flex px-1 py-2 text-sm dark:hover:bg-transparent">
            <ChevronLeft class="h-5 w-5 rtl:rotate-180" /> {t("back_to_bookings")}
          </Link>
        </div>
      )}
      <HeadSeo title={title} description={title} />
      <BookingPageTagManager eventType={eventType} />
      <main class={classNames(shouldAlignCentrally ? "mx-auto" : "", isEmbed ? "" : "max-w-3xl")}>
        <div class={classNames("overflow-y-auto", isEmbed ? "" : "z-50 ")}>
          <div
            class={classNames(
            ...
            <div
              ...
              <div
                ...
                <div
                  ...
                  {needsConfirmation && !isCancelled && <Calendar class="text-emphasis h-5 w-5" />}
                  {isCancelled && <X class="h-5 w-5 text-red-600 dark:text-red-200" />}
                </div>
                <div class="mb-8 mt-6 text-center last:mb-0">
                  ...
                  <div class="mt-3">
                    <p class="text-default">{getTitle()}</p>
                  </div>
                  {props.paymentStatus &&
                  ...
                  <div class="border-subtle text-default mt-8 grid grid-cols-3 border-t pt-8 text-left rtl:text-right">
                    {(isCancelled || reschedule) && cancellationReason && (
                      <>
                        <div class="font-medium">
                          {isCancelled ? t("reason") : t("reschedule_reason")}
                        </div>
                        <div class="col-span-2 mb-6 last:mb-0">{cancellationReason}</div>
                      </>
                    )}
                    <div class="font-medium">{t("what")}</div>
                    <div class="col-span-2 mb-6 last:mb-0" data-testid="booking-title">
                      {eventName}
                    </div>
                    <div class="font-medium">{t("when")}</div>
                    ...
                    {(bookingInfo?.user || bookingInfo?.attendees) && (
                      <>
                        <div class="font-medium">{t("who")}</div>
                        <div class="col-span-2 last:mb-0">
                          {bookingInfo?.user && (
                            <div class="mb-3">
                              <div>
                                <span data-testid="booking-host-name" class="mr-2">
                                  {bookingInfo.user.name}
                                </span>
                                <Badge variant="blue">{t("Host")}</Badge>
                              </div>
                              <p class="text-default">{bookingInfo.user.email}</p>
                            </div>
                          )}
                          {bookingInfo?.attendees.map((attendee) => (
                            <div key={attendee.name + attendee.email} class="mb-3 last:mb-0">
                              {attendee.name && (
                    ...
                    {locationToDisplay && !isCancelled && (
                      <>
                        <div class="mt-3 font-medium">{t("where")}</div>
                        <div class="col-span-2 mt-3" data-testid="where">
                    ...
                    {props.paymentStatus && (
                      <>
                        ...
                        <div class="col-span-2 mb-2 mt-3">
                    ...
                    {bookingInfo?.description && (
                      <>
                        <div class="mt-9 font-medium">{t("additional_notes")}</div>
                        <div class="col-span-2 mb-2 mt-9">
                          <p class="break-words">{bookingInfo.description}</p>
                        </div>
                      </>
                    )}
                  </div>
                  <div class="text-bookingdark dark:border-darkgray-200 mt-8 text-left dark:text-gray-300">
                    {Object.entries(bookingInfo.responses).map(([name, response]) => {
                      ...
                      return (
                        <>
                          <div class="text-emphasis mt-4 font-medium">{label}</div>
                          <p
                            class="text-default break-words"
                ...
                {requiresLoginToUpdate && (
                  <>
                    <hr class="border-subtle mb-8" />
                    <div class="text-center">
                      <span class="text-emphasis ltr:mr-2 rtl:ml-2">{t("need_to_make_a_change")}</span>
                      {/* Login button but redirect to here */}
                      <span class="text-default inline">
                        <span class="underline" data-testid="reschedule-link">
                          <Link
                ...
                {!requiresLoginToUpdate &&
                  ...
                  (isCancellationMode ? (
                    ...
                    <>
                      <hr class="border-subtle mb-8" />
                      <div class="text-center last:pb-0">
                        <span class="text-emphasis ltr:mr-2 rtl:ml-2">{t("need_to_make_a_change")}</span>
                        ...
                        <>
                          {!props.recurringBookings && (
                            <span class="text-default inline">
                              ...
                              <span class="mx-2">{t("or_lowercase")}</span>
                            </span>
                          )}
                ...
                {userIsOwner &&
                  ...
                  !!calculatedDuration && (
                    <>
                      <hr class="border-subtle mt-8" />
                      <div class="text-default align-center flex flex-row justify-center pt-8">
                        <span class="text-default flex self-center font-medium ltr:mr-2 rtl:ml-2 ">
                          {t("add_to_calendar")}
                        </span>
                        <div class="justify-left mt-1 flex text-left sm:mt-0">
                          <Link
                            ...
                            class="text-default border-subtle h-10 w-10 rounded-sm border px-3 py-2 ltr:mr-2 rtl:ml-2">
                            <svg
                              class="-mt-1.5 inline-block h-4 w-4"
                              fill="currentColor"
                              xmlns="http://www.w3.org/2000/svg"
                              viewBox="0 0 24 24">
                              <title>Google</title>
                          ...
                          <Link
                            ...
                            class="border-subtle text-default mx-2 h-10 w-10 rounded-sm border px-3 py-2"
                            target="_blank">
                            <svg
                              class="-mt-1.5 mr-1 inline-block h-4 w-4"
                              fill="currentColor"
                              xmlns="http://www.w3.org/2000/svg"
                              viewBox="0 0 24 24">
                          ...
                          <Link
                            ...
                            class="text-default border-subtle mx-2 h-10 w-10 rounded-sm border px-3 py-2"
                            target="_blank">
                            <svg
                              class="-mt-1.5 mr-1 inline-block h-4 w-4"
                              fill="currentColor"
                ...
                {session === null && !(userIsOwner || props.hideBranding) && (
                  <>
                    <hr class="border-subtle mt-8" />
                    <div class="text-default pt-8 text-center text-xs">
                      ...
                      <form
                        ...
                        <EmailInput
                          ...
                          id="email"
                          defaultValue={email}
                          class="mr- focus:border-brand-default border-default text-default mt-0 block w-full rounded-none rounded-l-md shadow-sm focus:ring-black sm:text-sm"
                          placeholder="rick.astley@cal.com"
                        />
                        <Button
                          size="lg"
                          type="submit"
                          class="min-w-max rounded-none rounded-r-md"
                          color="primary">
                          {t("try_for_free")}
                        </Button>
                      </form>
              ...
              {isGmail && (
                <Alert
                  class="main -mb-20 mt-4 inline-block ltr:text-left rtl:text-right sm:-mt-4 sm:mb-4 sm:w-full sm:max-w-xl sm:align-middle"
                  severity="warning"
                  message={
                    <div>
                      <p class="font-semibold">{t("google_new_spam_policy")}</p>
...
const DisplayLocation = ({
	...
	locationToDisplay.startsWith("http") ? (
		<a
			...
			rel="noreferrer">
			{providerName || "Link"}
			<ExternalLink class="text-default inline h-4 w-4" />
		</a>
	) : (
		<p class={className}>{locationToDisplay}</p>
	);
...
function RecurringBookings({
	...
	const recurringBookingsSorted = recurringBookings
		? recurringBookings.sort((a: ConfigType, b: ConfigType) =>
				...
				{eventType.recurringEvent?.count && (
					<span class="font-medium">
						{getEveryFreqFor({
							...
							recurringEvent: eventType.recurringEvent,
							recurringCount:
								recurringBookings?.length ?? undefined,
						})}
					</span>
				)}
				{eventType.recurringEvent?.count &&
				...
				{recurringBookingsSorted.length > 4 && (
					<Collapsible
						...
						<CollapsibleTrigger
							...
							+{" "}
							{t("plus_more", {
								count: recurringBookingsSorted.length - 4,
							})}
						</CollapsibleTrigger>
						<CollapsibleContent>
							{eventType.recurringEvent?.count &&
	...
	return (
		<div class={classNames(isCancelled ? "line-through" : "")}>
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
	...
	const maybeBookingUidFromSeat = await maybeGetBookingUidFromSeat(
		prisma,
		uid,
	);
	if (maybeBookingUidFromSeat.uid) uid = maybeBookingUidFromSeat.uid;
	if (maybeBookingUidFromSeat.seatReferenceUid)
		seatReferenceUid = maybeBookingUidFromSeat.seatReferenceUid;
	...
	bookingInfo["startTime"] = (
		bookingInfo?.startTime as Date
	)?.toISOString() as unknown as Date;
	bookingInfo["endTime"] = (
		bookingInfo?.endTime as Date
	)?.toISOString() as unknown as Date;
	...
	const eventType = {
		...
		customInputs: customInputSchema
			.array()
			.parse(eventTypeRaw.customInputs),
	};
	...
	const profile = {
		...
		brandColor: eventType.team
			? null
			: eventType.users[0].brandColor || null,
	...
	return {
		props: {
			themeBasis: eventType.team
				? eventType.team.slug
				: eventType.users[0]?.username,
			hideBranding: eventType.team
				? eventType.team.hideBranding
				: eventType.users[0].hideBranding,
			profile,
			eventType,
			recurringBookings: await getRecurringBookings(
				bookingInfo.recurringEventId,
			),
			trpcState: ssr.dehydrate(),
...
export const getServerSideProps = async (
	...
	const typeParam = Number.parseInt(asStringOrThrow(query.type));
	const ssr = await ssrInit(context);
	...
	const { eventType } = await ssr.viewer.eventTypes.get.fetch({
		id: typeParam,
	});
...
const EventSetupTab = dynamic(() =>
	import("@components/eventtype/EventSetupTab").then(
		(mod) => mod.EventSetupTab,
	),
);

const EventAvailabilityTab = dynamic(() =>
	import("@components/eventtype/EventAvailabilityTab").then(
		(mod) => mod.EventAvailabilityTab,
	),
);

const EventTeamTab = dynamic(() =>
	import("@components/eventtype/EventTeamTab").then(
		(mod) => mod.EventTeamTab,
	),
);

const EventLimitsTab = dynamic(() =>
	import("@components/eventtype/EventLimitsTab").then(
		(mod) => mod.EventLimitsTab,
	),
);

const EventAdvancedTab = dynamic(() =>
	import("@components/eventtype/EventAdvancedTab").then(
		(mod) => mod.EventAdvancedTab,
	),
);

const EventInstantTab = dynamic(() =>
	import("@components/eventtype/EventInstantTab").then(
		(mod) => mod.EventInstantTab,
	),
);

const EventRecurringTab = dynamic(() =>
	import("@components/eventtype/EventRecurringTab").then(
		(mod) => mod.EventRecurringTab,
	),
);

const EventAppsTab = dynamic(() =>
	import("@components/eventtype/EventAppsTab").then(
		(mod) => mod.EventAppsTab,
	),
);

const EventWorkflowsTab = dynamic(
	() => import("@components/eventtype/EventWorkfowsTab"),
);

const EventWebhooksTab = dynamic(() =>
	import("@components/eventtype/EventWebhooksTab").then(
		(mod) => mod.EventWebhooksTab,
	),
);

const ManagedEventTypeDialog = dynamic(
	() => import("@components/eventtype/ManagedEventDialog"),
);
...
const EventTypePage = (props: EventTypeSetupProps) => {
	...
	if (!!team && metadata !== null) {
		metadata.config = {
			...
			useHostSchedulesForTeamEvent:
				typeof eventType.metadata?.config
					?.useHostSchedulesForTeamEvent !== "undefined"
	...
	const defaultValues: any = useMemo(() => {
		return {
			...
			onlyShowFirstAvailableSlot:
				eventType.onlyShowFirstAvailableSlot || undefined,
	...
	if (appsMetadata) {
		numberOfActiveApps = Object.entries(appsMetadata).filter(
			([appId, appData]) =>
				eventTypeApps?.items.find((app) => app.slug === appId)
					?.isInstalled && appData.enabled,
		).length;
	}

	const permalink = `${CAL_URL}/${
	...
	const tabMap = {
		...
		availability: (
			<EventAvailabilityTab eventType={eventType} isTeamEvent={!!team} />
		),
		team: (
			<EventTeamTab
				teamMembers={teamMembers}
		...
		workflows: (
			<EventWorkflowsTab
				eventType={eventType}
				workflows={eventType.workflows.map(
					(workflowOnEventType) => workflowOnEventType.workflow,
				)}
			/>
		),
	...
	const handleSubmit = async (values: FormValues) => {
		...
		if (durationLimits) {
			...
			if (!isValid)
				throw new Error(t("event_setup_duration_limits_error"));
		}

		const layoutError = validateBookerLayouts(
			metadata?.bookerLayouts || null,
		);
		...
		if (metadata?.multipleDuration !== undefined) {
			if (metadata?.multipleDuration.length < 1) {
				...
				!length &&
				!metadata?.multipleDuration?.includes(length)
			) {
				throw new Error(
					t("event_setup_multiple_duration_default_error"),
				);
			}
		}
		...
		if (
			checkForMultiplePaymentApps(
				metadata as z.infer<typeof EventTypeMetaDataSchema>,
			)
		)
	...
	return (
		<>
			<EventTypeSingleLayout
				...
				eventType={eventType}
				activeWebhooksNumber={
					eventType.webhooks.filter((webhook) => webhook.active)
						.length
				}
				team={team}
				...
				<Form
					...
					handleSubmit={async (values) => {
						...
						if (!Number(length))
							throw new Error(t("event_setup_length_error"));

						if (bookingLimits) {
							const isValid =
								validateIntervalLimitOrder(bookingLimits);
							if (!isValid)
								throw new Error(
									t("event_setup_booking_limits_error"),
								);
						}

						if (durationLimits) {
							const isValid =
								validateIntervalLimitOrder(durationLimits);
							if (!isValid)
								throw new Error(
									t("event_setup_duration_limits_error"),
								);
						}
						...
						if (metadata?.multipleDuration !== undefined) {
							if (metadata?.multipleDuration.length < 1) {
								throw new Error(
									t("event_setup_multiple_duration_error"),
								);
							} else if (
								!length &&
								!metadata?.multipleDuration?.includes(length)
								...
								throw new Error(
									t(
										"event_setup_multiple_duration_default_error",
									),
								);
							}
						}
						...
						if (
							checkForMultiplePaymentApps(
								metadata as z.infer<
									typeof EventTypeMetaDataSchema
								>,
							)
						)
							throw new Error(
								t("event_setup_multiple_payment_apps_error"),
							);

						// eslint-disable-next-line @typescript-eslint/no-unused-vars
						const { availability, users, scheduleName, ...rest } =
							input;
						updateMutation.mutate({
							...rest,
							length,
			...
			{slugExistsChildrenDialogOpen.length ? (
				<ManagedEventTypeDialog
					...
					onConfirm={(e: { preventDefault: () => void }) => {
						...
						telemetry.event(
							telemetryEventTypes.slugReplacementAction,
						);
						setSlugExistsChildrenDialogOpen([]);
					}}
				/>
...
type GetByViewerResponse =
	| RouterOutputs["viewer"]["eventTypes"]["getByViewer"]
	| undefined;
...
const MobileTeamsTab: FC<MobileTeamsTabProps> = (props) => {
	...
	const tabs = eventTypeGroups.map((item) => ({
		...
		href: item.teamId
			? `/event-types?teamId=${item.teamId}`
			: "/event-types?noTeam",
		avatar: orgBranding
			? `${orgBranding.fullDomain}${item.teamId ? "/team" : ""}/${
					item.profile.slug
				}/avatar.png`
			: item.profile.image ??
				`${WEBAPP_URL + (item.teamId && "/team")}/${
					item.profile.slug
				}/avatar.png`,
	}));
	const { data } = useTypedQuery(querySchema);
	const events = eventTypeGroups.filter(
		(item) => item.teamId === data.teamId,
	);
		...
		<div>
			...
			{events.length > 0 ? (
				...
				<CreateFirstEventTypeView
					slug={eventTypeGroups[0].profile.slug ?? ""}
				/>
			)}
		</div>
	);
...
const Item = ({
	...
	group,
	readOnly,
}: { type: EventType; group: EventTypeGroup; readOnly: boolean }) => {
	const { t } = useLocale();

	const content = () => (
		<div>
			<span
				class="text-default font-semibold ltr:mr-1 rtl:ml-1"
				data-testid={`event-type-title-${type.id}`}>
				{type.title}
			</span>
			{group.profile.slug ? (
				<small
					class="text-subtle hidden font-normal leading-4 sm:inline"
	...
	return readOnly ? (
		<div class="flex-1 overflow-hidden pr-4 text-sm">
			{content()}
		...
		<Link
			...
			class="flex-1 overflow-hidden pr-4 text-sm"
			title={type.title}>
			<div>
				<span
					class="text-default font-semibold ltr:mr-1 rtl:ml-1"
...
export const EventTypeList = ({
	...
	return (
		<div class="bg-default border-subtle mb-16 flex overflow-hidden rounded-md border">
			<ul
				ref={parent}
				class="divide-subtle !static w-full divide-y"
				...
				{types.map((type, index) => {
					...
					return (
						<li key={type.id}>
							<div class="hover:bg-muted flex w-full items-center justify-between transition">
								<div class="group flex w-full max-w-full items-center justify-between overflow-hidden px-4 py-4 sm:px-6">
									{!(
									...
									<MemoizedItem
										...
										readOnly={readOnly}
									/>
									<div class="mt-4 hidden sm:mt-0 sm:flex">
										<div class="flex justify-between space-x-2 rtl:space-x-reverse">
											{type.team &&
												!isManagedEventType && (
													<UserAvatarGroup
													...
													<Dropdown modal={false}>
														<DropdownMenuTrigger
															...
															<Button
																...
																class="ltr:radix-state-open:rounded-r-md rtl:radix-state-open:rounded-l-md"
															/>
														</DropdownMenuTrigger>
														<DropdownMenuContent>
															{!readOnly && (
															...
															{!isManagedEventType &&
																!isChildrenManagedEventType && (
																	<>
																		<DropdownMenuItem class="outline-none">
																			<DropdownItem
																				type="button"
								...
								<div class="min-w-9 mx-5 flex sm:hidden">
									<Dropdown>
										<DropdownMenuTrigger
											asChild={true}
											data-testid={`event-type-options-${type.id}`}>
	...
	return (
		<div class="mb-4 flex items-center space-x-2">
			<Avatar
				...
				imageSrc={`${bookerUrl}${teamId ? "/team" : ""}/${
					profile.slug
				}/avatar.png`}
				size="md"
			...
			<div>
				<Link
					...
					class="text-emphasis font-bold">
					{profile?.name || ""}
				</Link>
				{membershipCount && teamId && (
					<span class="text-subtle relative -top-px me-2 ms-2 text-xs">
						<Link href={`/settings/teams/${teamId}/members`}>
							<Badge variant="gray">
								<Users class="-mt-px mr-1 inline h-3 w-3" />
								{membershipCount}
							</Badge>
						</Link>
					</span>
				...
				{profile?.slug && (
					<Link
						href={`${bookerUrl}/${profile.slug}`}
						class="text-subtle block text-xs">
						{`${bookerUrl
							.replace("https://", "")
							.replace("http://", "")}/${profile.slug}`}
					</Link>
				)}
			</div>
			{!profile?.slug && !!teamId && (
				<button onClick={() => publishTeamMutation.mutate({ teamId })}>
					<Badge variant="gray" class="-ml-2 mb-1">
						{t("upgrade")}
					</Badge>
				</button>
			)}
				...
				<>
					{isMobile ? (
						...
						data.eventTypeGroups.map(
							(group: EventTypeGroup, index: number) => (
								<div
									...
									<EventTypeListHeading
										...
										membershipCount={
											group.metadata.membershipCount
										}
										teamId={group.teamId}
...
const EventTypesPage: React.FC & {
	...
	const { data, status, error } = trpc.viewer.eventTypes.getByViewer.useQuery(
		filters && { filters },
		{
			refetchOnWindowFocus: false,
			gcTime: 1 * 60 * 60 * 1000,
...
export default function MeetingNotStarted(props: PageProps) {
  ...
  return (
    <>
      ...
      <main class="mx-auto my-24 max-w-3xl">
        <EmptyScreen
          Icon={Clock}
          headline={t("this_meeting_has_not_started_yet")}
          description={
            <>
              <h2 class="mb-2 text-center font-medium">{props.booking.title}</h2>
              <p class="text-subtle text-center">
                <Calendar class="-mt-1 mr-1 inline-block h-4 w-4" />
    ...
    <>
      ...
      <div style={{ zIndex: 2, position: "relative" }}>
        {booking?.user?.organization?.calVideoLogo ? (
          <img
            class="min-w-16 min-h-16 fixed z-10 hidden aspect-square h-16 w-16 rounded-full sm:inline-block"
          ...
          <img
            class="fixed z-10 hidden sm:inline-block"
	...
	return (
		<div>
			...
			<div class="relative h-2 max-w-xl overflow-hidden rounded-full">
				<div class="absolute h-full w-full bg-gray-500/10" />
				<div
					class={classNames("relative h-full bg-green-500")}
	...
	return (
		<>
			<aside
				...
				<main class="prose-sm prose max-w-64 prose-a:text-white prose-h3:text-white prose-h3:font-cal scroll-bar scrollbar-track-w-20 w-full overflow-scroll overflow-x-hidden border-r border-gray-300/20 bg-black/80 p-4 text-white shadow-sm backdrop-blur-lg">
					<h3>{t("what")}:</h3>
					<p>{booking.title}</p>
					...
					{booking.description && (
						<>
							...
							<div
								class="prose-sm prose prose-invert"
								dangerouslySetInnerHTML={{
				...
				<div class="flex items-center justify-center">
					<button
						...
						class="h-20 w-6 rounded-r-md border border-l-0 border-gray-300/20 bg-black/60 text-white shadow-sm backdrop-blur-lg"
...
import { useLocale } from "@calcom/lib/hooks/useLocale";
import { Button, EmptyScreen, HeadSeo } from "@calcom/ui";
import { ArrowRight, X } from "@calcom/ui/components/icon";
    ...
    <div>
      ...
      <main class="mx-auto my-24 max-w-3xl">
        <div class="fixed inset-0 z-50 overflow-y-auto">
          <div class="flex min-h-screen items-end justify-center px-4 pb-20 pt-4 text-center sm:block sm:p-0">
            <div class="fixed inset-0 my-4 transition-opacity sm:my-0" aria-hidden="true">
              <span class="hidden sm:inline-block sm:h-screen sm:align-middle" aria-hidden="true">
                &#8203;
              </span>
              <div
                class="bg-default inline-block transform overflow-hidden rounded-lg px-4 pb-4 pt-5 text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6 sm:align-middle"
                role="dialog"
                aria-modal="true"
                aria-labelledby="modal-headline">
                <div>
                  ...
                  <div class="mt-3 text-center sm:mt-5">
                    <h3 class="text-emphasis text-lg font-medium leading-6" id="modal-headline">
                      This meeting is in the past.
                    </h3>
                  </div>
                  <div class="mt-4 border-b border-t py-4">
                    <h2 class="font-cal text-default mb-2 text-center text-lg font-medium">
                      {props.booking.title}
                    </h2>
                    <p class="text-subtle text-center">
                      <Calendar class="-mt-1 mr-1 inline-block h-4 w-4" />
...
import { stripMarkdown } from "@calcom/lib/stripMarkdown";
import prisma from "@calcom/prisma";
import { type EventType, RedirectType, type User } from "@calcom/prisma/client";
...
export type UserPageProps = {
	...
	users: Pick<
		User,
		"away" | "name" | "username" | "bio" | "verified" | "avatarUrl"
	>[];
	themeBasis: string | null;
...
export const getServerSideProps: GetServerSideProps<UserPageProps> = async (
	...
	if (usernameList.length === 1) {
		const result = await handleUserRedirection({
	...
	const users = usersWithoutAvatar.map((user) => ({
		...
		organization: {
			...user.organization,
			metadata: user.organization?.metadata
				? teamMetadataSchema.parse(user.organization.metadata)
				: null,
		},
		avatar: `/${user.username}/avatar.png`,
	}));
	...
	if (isDynamicGroup) {
		...
		logger.debug(
			`Dynamic group detected, redirecting to ${destinationUrl}`,
		);
		return {
			redirect: {
				permanent: false,
	...
	if (
		!users.length ||
		(!isValidOrgDomain &&
			!users.some((user) => user.organizationId === null))
	) {
		return {
			notFound: true,
		} as {
...
export function UserPage(
   ...
   useEffect(() => {
	...
	if (entity?.isUnpublished) {
		return (
			<div class="flex h-full min-h-[100dvh] items-center justify-center">
			...
			<div
				...
				<main
					class={classNames(
						shouldAlignCentrally ? "mx-auto" : "",
						isEmbed
							? "border-booker border-booker-width  bg-default rounded-md border"
							: "",
						"max-w-3xl px-4 py-24",
					)}>
					{isRedirect && (
						<div class="mb-8 rounded-md bg-blue-100 p-4 dark:border dark:bg-transparent dark:bg-transparent">
							<h2 class="text-default mb-2 text-sm font-semibold dark:text-white">
					...
					<div class="mb-8 text-center">
						...
						/>
						<h1
							class="font-cal text-emphasis my-1 text-3xl"
							data-testid="name-title">
							{profile.name}
							{user.verified && (
								<Verified class=" mx-1 -mt-1 inline h-6 w-6 fill-blue-500 text-white dark:text-black" />
							)}
						</h1>
						{!isBioEmpty && (
							<>
						...
						{user.away ? (
							<div class="overflow-hidden rounded-sm border ">
								<div class="text-muted  p-8 text-center">
									<h2 class="font-cal text-default mb-2 text-3xl">
										😴{` ${t("user_away")}`}
									</h2>
							...
							eventTypes.map((type) => (
								<div
									...
									class="bg-default border-subtle dark:bg-muted dark:hover:bg-emphasis hover:bg-muted group relative border-b first:rounded-t-md last:rounded-b-md last:border-b-0">
									<ArrowRight class="text-emphasis  absolute right-4 top-4 h-4 w-4 opacity-0 transition-opacity group-hover:opacity-100" />
...
export default function Type({
  ...
  return (
    <main class={getBookerWrapperClasses({ isEmbed: !!isEmbed })}>
      <BookerSeo
        username={user}
        eventSlug={slug}
          ...
          {user ? (
            <FiltersProvider>
              ...
              <div class="mb-4 space-y-4">
                ...
                <small class="text-default block text-center">
...
OnboardTeamMembersPage.getLayout = (page: React.ReactElement) => {
	...
	return (
		<WizardLayout
			...
			isOptionalCallback={() => {
				router.push(
					`/settings/organizations/${query.get("id")}/add-teams`,
				);
			}}>
			{page}
		</WizardLayout>
...
const SkeletonLoader = ({
	...
	return (
		<SkeletonContainer>
			...
			/>
			<div class="divide-subtle border-subtle space-y-6 rounded-b-lg border border-t-0 px-6 py-4">
...
const ApiKeysView = () => {
	...
	const [apiKeyToEdit, setApiKeyToEdit] = useState<
		(TApiKeys & { neverExpires?: boolean }) | undefined
	>(undefined);
	...
	return (
		<SkeletonContainer>
			<Meta
				...
				borderInShellHeader={true}
			/>
			<div class="border-subtle space-y-6 border border-t-0 px-4 py-8 sm:px-6">
				<SkeletonText class="h-8 w-full" />
			</div>
		</SkeletonContainer>
	);
	...
	const mutation = trpc.viewer.updateProfile.useMutation({
		...
		onError: (error, variables, context) => {
			if (context?.previousValue) {
				...
				setDisableImpersonation(
					context.previousValue?.disableImpersonation,
				);
			}
...
const TwoFactorAuthView = () => {
	...
	return (
		<>
			...
			/>
			{canSetupTwoFactor && (
				<Alert severity="neutral" message={t("2fa_disabled")} />
			)}
			<SettingsToggle
				toggleSwitchAtTheEnd={true}
	...
	return (
		<SkeletonContainer>
			<Meta
				...
				borderInShellHeader={true}
			/>
			<div class="border-subtle space-y-6 border-x px-4 py-8 sm:px-6">
				<SkeletonText class="h-8 w-full" />
				<SkeletonText class="h-8 w-full" />
				<SkeletonText class="h-8 w-full" />
			</div>
			<div class="rounded-b-xl">
				<SectionBottomActions align="end">
					<SkeletonButton class="ml-auto h-8 w-20 rounded-md" />
	...
	return (
		<>
			...
			{user && user.identityProvider !== IdentityProvider.CAL ? (
				<div class="border-subtle rounded-b-xl border border-t-0 px-4 py-6 sm:px-6">
					<h2 class="font-cal text-emphasis text-lg font-medium leading-6">
				...
				<Form form={formMethods} handleSubmit={handleSubmit}>
					<div class="border-subtle border-x px-4 py-6 sm:px-6">
						...
						<div class="w-full sm:grid sm:grid-cols-2 sm:gap-x-6">
							<div>
								<PasswordField
						...
						<p class="text-default mt-4 w-full text-sm">
					...
					<div class="mt-6">
						<SettingsToggle
							...
							<>
								<div class="border-subtle border-x p-6 pb-8">
									<div class="flex flex-col">
										<p class="text-default mb-2 font-medium">
										...
										<Select
											...
											isSearchable={false}
											class="block h-[36px] !w-auto min-w-0 flex-none rounded-md text-sm"
											onChange={(event) => {
      ...
      <Form
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            {moveMembers === true ? (
              <div class="mt-2">Members of the team will also be moved to the organization</div>
            ) : moveMembers === false ? (
              <div class="mt-2">Members of the team will not be moved to the organization</div>
            ) : null}
          </div>
        </div>
        <Button type="submit" loading={state === State.LOADING}>
...
export const getFormSchema = (t: TFunction) =>
	z.object({
		userId: z
			.union([z.string().pipe(z.coerce.number()), z.number()])
			.optional(),
...
export default function OAuthView() {
  ...
  return (
    <div>
      ...
      {clientId ? (
        <div>
          <div class="text-emphasis mb-5 text-xl font-semibold">{oAuthForm.getValues("name")}</div>
          <div class="mb-2 font-medium">Client Id</div>
          <div class="flex">
            <code class="bg-subtle text-default w-full truncate rounded-md rounded-r-none py-[6px] pl-2 pr-2 align-middle font-mono">
              {" "}
              {clientId}
            </code>
          ...
          {clientSecret ? (
            <>
              <div class="mb-2 mt-4 font-medium">Client Secret</div>
              <div class="flex">
                <code class="bg-subtle text-default w-full truncate rounded-md rounded-r-none py-[6px] pl-2 pr-2 align-middle font-mono">
                  {" "}
                  {clientSecret}
              ...
              <div class="text-subtle text-sm">{t("copy_client_secret_info")}</div>
            </>
          ) : (
            <></>
          )}
        ...
        <Form
          ...
          <div class="">
            ...
            <div class="mb-5 mt-5 flex items-center">
              <Avatar
                alt=""
                fallback={<Plus class="text-subtle h-6 w-6" />}
	...
	return (
		<SkeletonContainer>
			<Meta
				...
				borderInShellHeader={true}
			/>
			<div class="border-subtle space-y-6 rounded-b-lg border border-t-0 px-4 py-8">
				<div class="flex items-center">
					<SkeletonAvatar class="me-4 mt-0 h-16 w-16 px-4" />
					<SkeletonButton class="h-6 w-32 rounded-md p-5" />
				</div>
				<SkeletonText class="h-8 w-full" />
				<SkeletonText class="h-8 w-full" />
				<SkeletonText class="h-8 w-full" />
...
const ProfileView = () => {
	...
	const updateProfileMutation = trpc.viewer.updateProfile.useMutation({
		onSuccess: async (res) => {
			...
			if (res.signOutUser && tempFormValues && res.passwordReset) {
				showToast(
					t("password_reset_email", { email: tempFormValues.email }),
					"success",
				);
	...
	const [tempFormValues, setTempFormValues] = useState<FormValues | null>(
		null,
	);
	const [confirmPasswordErrorMessage, setConfirmPasswordDeleteErrorMessage] =
		useState("");
	const [
		confirmAuthEmailChangeWarningDialogOpen,
	...
	const confirmPasswordMutation = trpc.viewer.auth.verifyPassword.useMutation(
		{
			onSuccess() {
				if (tempFormValues)
					updateProfileMutation.mutate(tempFormValues);
	...
	const onConfirmAuthEmailChange = (
		e: Event | React.MouseEvent<HTMLElement, MouseEvent>,
	) => {
		e.preventDefault();
	...
	return (
		<>
			...
			<div class="border-subtle mt-6 rounded-lg rounded-b-none border border-b-0 p-6">
				<Label class="mb-0 text-base font-semibold text-red-700">
					{t("danger_zone")}
				</Label>
				<p class="text-subtle text-sm">
...
const ProfileForm = ({
	...
	return (
		<Form form={formMethods} handleSubmit={onSubmit}>
			<div class="border-subtle border-x px-4 pb-10 pt-8 sm:px-6">
				<div class="flex items-center">
					<Controller
						control={formMethods.control}
						name="avatar"
						render={({ field: { value } }) => {
							const showRemoveAvatarButton =
								value === null ? false : !isFallbackImg;
							const organization =
								userOrganization && userOrganization.id
									? {
											...(userOrganization as Ensure<
												typeof user.organization,
												"id"
											>),
											slug: userOrganization.slug || null,
											requestedSlug:
												userOrganization.metadata
													?.requestedSlug || null,
										}
									: null;
							return (
	...
	return (
		<>
			<form
				...
				<div class="border-subtle flex flex-col border border-b-0 border-t-0 p-6 px-6 py-8 text-sm">
					...
					<div class="flex flex-row">
						...
						/>
						{!hasTeamPlan && (
							<div class="mx-2" data-testid="upgrade-team-badge">
								<UpgradeTeamsBadge />
							</div>
						)}
					</div>
					<div class="mt-4 grid grid-rows-2 gap-2 md:grid-cols-2">
						{profileRedirect && (
							<div>
								<p class="text-emphasis block text-sm font-medium">
									{t("team_member")}
								</p>
								<Select
									class="mt-1 h-4 text-white"
									name="toTeamUsername"
						...
						<div>
							<p class="text-emphasis mb-1 block text-sm font-medium">
								{t("time_range")}
							</p>
							<div>
								<Controller
									...
									render={() => (
										<DateRangePicker
											startDate={
												getValues("dateRange").startDate
											}
											endDate={
												getValues("dateRange").endDate
											}
											onDatesChange={({
												startDate,
												endDate,
				...
				<SectionBottomActions class="mb-6" align="end">
					<Button
						color="primary"
						type="submit"
...
const OutOfOfficeEntriesList = () => {
	...
	const deleteOutOfOfficeEntryMutation =
	...
	return (
		<div class="border-subtle mt-6 rounded-lg border">
			<TableNew class="border-0">
				<TableHeader class="md:z-1">
					<TableRow>
						<TableHead class="rounded-tl-lg font-normal capitalize">
							{t("time_range")}
						</TableHead>
						<TableHead class="font-normal">
							{t("redirect_to")}
						</TableHead>

						<TableHead class="rounded-tr-lg font-normal">
							{t("action")}
						</TableHead>
					</TableRow>
				</TableHeader>
				<TableBody>
					{data?.map((item) => (
						<TableRow
							key={item.id}
							data-testid={`table-redirect-${
								item.toUser?.username || "n-a"
							}`}>
							<TableCell>
								<p class="px-2">
									{dayjs.utc(item.start).format("ll")} -{" "}
									{dayjs.utc(item.end).format("ll")}
								</p>
							</TableCell>
							<TableCell>
								<p class="px-2">
...
const ConferencingLayout = () => {
	...
	return (
		<>
			<div class="bg-default w-full sm:mx-0 xl:mt-0">
				<Meta
						...
						if (!data.items.length) {
							return (
								<EmptyScreen
									...
									headline={t("no_category_apps", {
										category:
											t("conferencing").toLowerCase(),
									})}
									description={t(
										"no_category_apps_description_conferencing",
									)}
									buttonRaw={
										<Button
											color="secondary"
...
const SkeletonLoader = ({
	...
	return (
		<SkeletonContainer>
			...
			/>
			<div class="border-subtle mt-6 flex items-center rounded-t-xl border p-6 text-sm">
				<SkeletonText class="h-8 w-1/3" />
			</div>
			<div class="border-subtle space-y-6 border-x px-4 py-6 sm:px-6">
				<div class="flex items-center justify-center">
					<SkeletonButton class="mr-6 h-32 w-48 rounded-md p-5" />
					<SkeletonButton class="mr-6 h-32 w-48 rounded-md p-5" />
					<SkeletonButton class="mr-6 h-32 w-48 rounded-md p-5" />
				</div>
				<div class="flex justify-between">
	...
	return (
		<div>
			<Meta
				...
				borderInShellHeader={false}
			/>
			<div class="border-subtle mt-6 flex items-center rounded-t-lg border p-6 text-sm">
				<div>
					<p class="text-default text-base font-semibold">
						{t("theme")}
					</p>
					<p class="text-default">{t("theme_applies_note")}</p>
				</div>
			</div>
			<Form
				...
				<div class="border-subtle flex flex-col justify-between border-x px-6 py-8 sm:flex-row">
					<ThemeLabel
						variant="system"
						value={undefined}
			...
			<Form
				...
				<div class="mt-6">
					<SettingsToggle
						...
						childrenClassName="lg:ml-0">
						<div class="border-subtle flex flex-col gap-6 border-x p-6">
							<Controller
								name="brandColor"
								...
								render={() => (
									<div>
										<p class="text-default mb-2 block text-sm font-medium">
											{t("light_brand_color")}
										</p>
										<ColorPicker
											defaultValue={
												DEFAULT_BRAND_COLOURS.light
											}
											resetDefaultValue={
												DEFAULT_LIGHT_BRAND_COLOR
											}
...
const AppearanceViewWrapper = () => {
	const { data: user, isPending } = trpc.viewer.me.useQuery();
	const { isPending: isTeamPlanStatusLoading, hasPaidPlan } =
		useHasPaidPlan();
...
const SkeletonLoader = () => {
	return (
		<SkeletonContainer>
			<div class="border-subtle mt-8 space-y-6 rounded-lg border px-4 py-6 sm:px-6">
				<SkeletonText class="h-8 w-full" />
				<SkeletonText class="h-8 w-full" />
				<SkeletonText class="h-8 w-full" />
				<SkeletonText class="h-8 w-full" />

				<SkeletonButton class="ml-auto h-8 w-20 rounded-md p-5" />
			</div>
		</SkeletonContainer>
	);
};
	...
	return (
		<SkeletonContainer>
			<Meta
				...
				borderInShellHeader={true}
			/>
			<div class="border-subtle space-y-6 rounded-b-xl border border-t-0 px-4 py-8 sm:px-6">
				<SkeletonText class="h-8 w-full" />
				<SkeletonText class="h-8 w-full" />
				<SkeletonText class="h-8 w-full" />
				<SkeletonText class="h-8 w-full" />
...
const GeneralView = ({ localeProp, user }: GeneralViewProps) => {
	...
	const { t } = useLocale();
	const { update } = useSession();
	const [isUpdateBtnLoading, setIsUpdateBtnLoading] =
		useState<boolean>(false);
	...
	const formMethods = useForm({
		defaultValues: {
			locale: {
				value: localeProp || "",
				label:
					localeOptions.find((option) => option.value === localeProp)
						?.label || "",
			},
			...
			timeFormat: {
				...
				label:
					timeFormatOptions.find(
						(option) => option.value === user.timeFormat,
					)?.label || 12,
			},
			weekStart: {
		...
		<div>
			<Form
				...
				<Meta
					...
					borderInShellHeader={true}
				/>
				<div class="border-subtle border-x border-y-0 px-4 py-8 sm:px-6">
					<Controller
						name="locale"
						render={({ field: { value, onChange } }) => (
							<>
					...
					<Controller
						...
						render={({ field: { value } }) => (
							<>
								<Label class="text-emphasis mt-6">
									<>{t("timezone")}</>
								</Label>
								<TimezoneSelect
									id="timezone"
					...
					/>
					<div class="text-gray text-default mt-2 flex items-center text-sm">
	...
	return (
		<>
			<Head>
				...
				<meta
					name="description"
					content={t("create_new_team_description")}
				/>
			</Head>
			<CreateANewTeamForm />
		</>
	...
	return (
		<>
			<Head>
				...
				<meta
					name="description"
					content={t("add_team_members_description")}
				/>
			</Head>
			<AddNewTeamMembers />
		</>
	...
	return (
		<>
			<section
				...
				<div class="flex-shrink-0 pt-3 sm:ml-auto sm:pl-3 sm:pt-0">
					{children}
				</div>
			</section>
		</>
	...
	return (
		<>
			<Meta
				...
				borderInShellHeader={true}
			/>
			<div class="border-subtle space-y-6 rounded-b-lg border border-t-0 px-6 py-8 text-sm sm:space-y-8">
				<CtaRow
				...
				<CtaRow
					title={t("need_anything_else")}
					description={t("further_billing_help")}>
...
export {
	getServerSideProps,
	type PageProps,
} from "~/videos/views/videos-single-view.getServerSideProps";
...
export default function MaintenancePage() {
  ...
  return (
    <div class="bg-subtle flex h-screen">
      <Head>
        <title>
      ...
      <div class="bg-default m-auto rounded-md p-10 text-right ltr:text-left">
        <h1 class="text-emphasis text-2xl font-medium">{t("under_maintenance")}</h1>
        <p class="text-default mb-6 mt-4 max-w-2xl text-sm">
...
export default function Custom404() {
  ...
  if (isInsights) {
    return (
      <>
        ...
        />
        <div class="min-h-screen bg-white px-4" data-testid="404-page">
          <main class="mx-auto max-w-xl pb-6 pt-16 sm:pt-24">
            <div class="text-center">
              <p class="text-sm font-semibold uppercase tracking-wide text-black">{t("error_404")}</p>
              <h1 class="font-cal mt-2 text-4xl font-extrabold text-gray-900 sm:text-5xl">
                Feature is currently disabled
              </h1>
            </div>
            <div class="mt-12">
              <div class="mt-8">
                <Link href={WEBSITE_URL} class="text-base font-medium text-black hover:text-gray-500">
                  {t("or_go_back_home")}
  ...
  return (
    <>
      ...
      />
      <div class="bg-default min-h-screen px-4" data-testid="404-page">
        <main class="mx-auto max-w-xl pb-6 pt-16 sm:pt-24">
          <div class="text-center">
            <p class="text-emphasis text-sm font-semibold uppercase tracking-wide">{t("error_404")}</p>
            <h1 class="font-cal text-emphasis mt-2 text-4xl font-extrabold sm:text-5xl">
              {isSuccessPage ? "Booking not found" : t("page_doesnt_exist")}
            </h1>
            {isSubpage && currentPageType !== pageType.TEAM ? (
              <span class="mt-2 inline-block text-lg ">{t("check_spelling_mistakes_or_go_back")}</span>
            ) : IS_CALCOM ? (
              <a target="_blank" href={url} class="mt-2 inline-block text-lg" rel="noreferrer">
                {t(`404_the_${currentPageType.toLowerCase()}`)}{" "}
                <strong class="text-blue-500">{username}</strong> {t("is_still_available")}{" "}
                <span class="text-blue-500">{t("register_now")}</span>.
              </a>
            ) : (
              <span class="mt-2 inline-block text-lg">
                {t(`404_the_${currentPageType.toLowerCase()}`)}{" "}
                <strong class="text-lgtext-green-500 mt-2 inline-block">{username}</strong>{" "}
          ...
          <div class="mt-12">
            {((!isSubpage && IS_CALCOM) ||
              ...
              <ul role="list" class="my-4">
                <li class="border-2 border-green-500 px-4 py-2">
                  <a
                    href={url}
                    target="_blank"
                    class="relative flex items-start space-x-4 py-6 rtl:space-x-reverse"
                    rel="noreferrer">
                    ...
                    <div class="min-w-0 flex-1">
                      <h3 class="text-emphasis text-base font-medium">
                        <span class="focus-within:ring-empthasis rounded-sm focus-within:ring-2 focus-within:ring-offset-2">
                      ...
                      <p class="text-subtle text-base">
                        {t(`404_claim_entity_${currentPageType.toLowerCase()}`)}
                      </p>
                    </div>
                    <div class="flex-shrink-0 self-center">
                      <ChevronRight class="text-muted h-5 w-5" aria-hidden="true" />
                    </div>
                  </a>
                </li>
              </ul>
            )}
            <h2 class="text-subtle text-sm font-semibold uppercase tracking-wide">
              {t("popular_pages")}
            </h2>
            <ul role="list" class="border-subtle divide-subtle divide-y">
              {links
                .filter((_, idx) => currentPageType === pageType.ORG || idx !== 0)
                .map((link, linkIdx) => (
                  <li key={linkIdx} class="px-4 py-2">
                    <a
                      href={link.href}
                      class="relative flex items-start space-x-4 py-6 rtl:space-x-reverse">
                      <div class="flex-shrink-0">
                        <span class="bg-muted flex h-12 w-12 items-center justify-center rounded-lg">
                          <link.icon class="text-default h-6 w-6" aria-hidden="true" />
                        </span>
                      </div>
                      <div class="min-w-0 flex-1">
                        <h3 class="text-emphasis text-base font-medium">
                        ...
                        <p class="text-subtle text-base">{link.description}</p>
                      </div>
                      <div class="flex-shrink-0 self-center">
                        <ChevronRight class="text-muted h-5 w-5" aria-hidden="true" />
                      </div>
              ...
              <li class="px-4 py-2">
                <a
                  ...
                  <div class="flex-shrink-0">
                    <span class="bg-muted flex h-12 w-12 items-center justify-center rounded-lg">
                      <Discord class="text-default h-6 w-6" />
                    </span>
                  </div>
                  <div class="min-w-0 flex-1">
                    <h3 class="text-emphasis text-base font-medium">
                    ...
                    <p class="text-subtle text-base">{t("join_our_community")}</p>
                  </div>
                  <div class="flex-shrink-0 self-center">
                    <ChevronRight class="text-muted h-5 w-5" aria-hidden="true" />
                  </div>
            ...
            <div class="mt-8">
              <Link href={WEBSITE_URL} class="hover:text-subtle text-emphasis text-base font-medium">
                {t("or_go_back_home")}
                <span aria-hidden="true"> &rarr;</span>
              </Link>
            </div>
...
inferSSRProps<typeof getServerSideProps> & WithNonceProps<{}>) {
  ...
  const LoginFooter = (
    <Link href={`${WEBSITE_URL}/signup`} class="text-brand-500 font-medium">
  ...
  return (
    <div class="dark:bg-brand dark:text-brand-contrast text-emphasis min-h-screen [--cal-brand-emphasis:#101010] [--cal-brand-subtle:#9CA3AF] [--cal-brand-text:white] [--cal-brand:#111827] dark:[--cal-brand-emphasis:#e1e1e1] dark:[--cal-brand-text:black] dark:[--cal-brand:white]">
      <AuthContainer
        title={t("login")}
        description={t("login")}
        showLogo={true}
        ...
        footerText={
          twoFactorRequired
            ? totpEmail
              ? ExternalTotpFooter : TwoFactorFooter
            : process.env.NEXT_PUBLIC_DISABLE_SIGNUP !== "true"
            ? LoginFooter
            : null
        }>
        <FormProvider {...methods}>
          <form onSubmit={methods.handleSubmit(onSubmit)} noValidate={true} data-testid="login-form">
            <div>
              <input defaultValue={csrfToken || undefined} type="hidden" hidden={true} {...register("csrfToken")} />
            </div>
            <div class="space-y-6">
              <div class={classNames("space-y-6", { hidden: twoFactorRequired })}>
                <EmailField
                  id="email"
                ...
                <div class="relative">
                  <PasswordField
                    ...
                    {...register("password")}
                  />
                  <div class="absolute -top-[2px] ltr:right-0 rtl:left-0">
                    <Link
              ...
              {twoFactorRequired ? twoFactorLostAccess ? <BackupCode center={true} />  : <TwoFactor center={true} /> : null}
          ...
          {!twoFactorRequired && (
            <>
              {(isGoogleLoginEnabled || displaySSOLogin) && <hr class="border-subtle my-8" />}
              <div class="space-y-3">
                {isGoogleLoginEnabled && (
                  <Button
                    color="secondary"
...
export function Setup(props: inferSSRProps<typeof getServerSideProps>) {
			...
			return (
				<AdminAppsList
					...
					classNames={{
						...
						appCategoryNavigationContainer:
							"max-h-[400px] overflow-y-auto md:p-4",
	...
	return (
		<>
			...
			<main class="bg-subtle flex items-center print:h-full md:h-screen">
				<WizardForm
					href="/auth/setup"
					steps={steps}
...
export default function Authorize() {
  ...
  return (
    <div class="flex min-h-screen items-center justify-center">
      <div class="bg-default border-subtle mt-2 max-w-xl rounded-md border px-9 pb-3 pt-2">
        <div class="flex items-center justify-center">
          <Avatar
            alt=""
            fallback={<Plus class="text-subtle h-6 w-6" />}
            ...
            size="lg"
          />
          <div class="relative -ml-6 h-24 w-24">
            <div class="absolute inset-0 flex items-center justify-center">
              <div class="bg-default flex h-[70px] w-[70px] items-center  justify-center rounded-full">
                <img src="/cal-com-icon.svg" alt="Logo" class="h-16 w-16 rounded-full" />
              </div>
            </div>
          </div>
        </div>
        <h1 class="px-5 pb-5 pt-3 text-center text-2xl font-bold tracking-tight">
          {t("access_cal_account", { clientName: client.name, appName: APP_NAME })}
        </h1>
        <div class="mb-1 text-sm font-medium">{t("select_account_team")}</div>
        <Select
          isSearchable={true}
          id="account-select"
          onChange={(value) => {
        ...
        />
        <div class="mb-4 mt-5 font-medium">{t("allow_client_to", { clientName: client.name })}</div>
        <ul class="space-y-4 text-sm">
          ...
          <li class="relative pl-5">
            <span class="absolute left-0">&#10003;</span> {t("see_personal_info")}
          </li>
          <li class="relative pl-5">
            <span class="absolute left-0">&#10003;</span> {t("see_primary_email_address")}
          </li>
          <li class="relative pl-5">
            <span class="absolute left-0">&#10003;</span> {t("connect_installed_apps")}
          </li>
          <li class="relative pl-5">
            <span class="absolute left-0">&#10003;</span> {t("access_event_type")}
          </li>
          <li class="relative pl-5">
            <span class="absolute left-0">&#10003;</span> {t("access_availability")}
          </li>
          <li class="relative pl-5">
            <span class="absolute left-0">&#10003;</span> {t("access_bookings")}
          </li>
        </ul>
        <div class="bg-subtle mb-8 mt-8 flex rounded-md p-3">
          <div>
            <Info class="mr-1 mt-0.5 h-4 w-4" />
          </div>
          <div class="ml-1 ">
            <div class="mb-1 text-sm font-medium">
              {t("allow_client_to_do", { clientName: client.name })}
            </div>
            <div class="text-sm">{t("oauth_access_information", { appName: APP_NAME })}</div>{" "}
          </div>
        </div>
        <div class="border-subtle border- -mx-9 mb-4 border-b" />
        <div class="flex justify-end">
          <Button
            class="mr-2"
            color="minimal"
		...
		<AuthContainer
			...
			<div class="mb-4">
				...
				<div class="mt-3 text-center sm:mt-5">
					...
					<div class="mt-2">
						<p class="text-subtle text-sm">{t(message())}</p>
					</div>
				</div>
			</div>
			<Button
...
export default function ForgotPassword(props: inferSSRProps<typeof getServerSideProps>) {
  ...
  const Success = () => {
    return (
      <div class="space-y-6 text-sm leading-normal ">
        <p class="">{t("password_reset_email", { email })}</p>
        <p class="">{t("password_reset_leading")}</p>
        {error && <p class="text-center text-red-600">{error.message}</p>}
        <Button color="secondary" class="w-full justify-center" href="/auth/login">
          {t("back_to_signin")}
        </Button>
      </div>
    );
  ...
  return (
    <AuthContainer
      showLogo={true}
      title={success ? t("reset_link_sent") : t("forgot_password")}
      heading={success ? t("reset_link_sent") : t("forgot_password")}
      description={t("request_password_reset")}
      footerText={
        !success && (
          <>
            <Link href="/auth/login" class="text-emphasis font-medium">
              {t("back_to_signin")}
            </Link>
          </>
        )
      ...
      {!success && (
        <>
          <div class="space-y-6">{error && <p class="text-red-600">{error.message}</p>}</div>
          <form
            class="space-y-6"
            onSubmit={handleSubmit}
            action="#"
            ...
            <input name="csrfToken" type="hidden" defaultValue={csrfToken} hidden={true} />
            <EmailField
            ...
            <div class="space-y-2">
              <Button
                class="w-full justify-center dark:bg-white dark:text-black"
                type="submit"
                color="primary"
                disabled={loading}
...
export default function Page({ requestId, isRequestExpired, csrfToken }: Props) {
  ...
  const Success = () => {
    return (
      <>
        <div class="space-y-6">
          <div>
            <h2 class="font-cal text-emphasis mt-6 text-center text-3xl font-extrabold">
              {t("password_updated")}
            </h2>
          </div>
          <Button href="/auth/login" class="w-full justify-center">
            {t("login")}
          </Button>
        </div>
      </>
  ...
  const Expired = () => {
    return (
      <>
        <div class="space-y-6">
          <div>
            <h2 class="font-cal text-emphasis mt-6 text-center text-3xl font-extrabold">{t("whoops")}</h2>
            <h2 class="text-emphasis text-center text-3xl font-extrabold">{t("request_is_expired")}</h2>
          </div>
          <p>{t("request_is_expired_instructions")}</p>
          <Link href="/auth/forgot-password" passHref={true} legacyBehavior={true}>
            <button
              type="button"
              class="flex w-full justify-center px-4 py-2 text-sm font-medium text-blue-600 focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2">
              {t("try_again")}
            </button>
          </Link>
        </div>
  ...
  return (
    <AuthContainer
      ...
      heading={success ? undefined : t("reset_password")}>
    ...
    <AuthContainer title="" description="">
      <div>
        ...
        <div class="mt-3 text-center sm:mt-5">
          ...
          <div class="mt-2">
            <p class="text-subtle text-sm">{errorMsg}</p>
          </div>
        </div>
      </div>
      <div class="mt-5 sm:mt-6">
        <Link href="/auth/login" passHref={true} legacyBehavior={true}>
          <Button class="flex w-full justify-center">{t("go_back_login")}</Button>
        </Link>
      </div>
    </AuthContainer>
  );
...
const PaymentFailedIcon = () => (
	<div class="rounded-full bg-orange-900 p-3">
		<AlertTriangle class="h-6 w-6 flex-shrink-0 p-0.5 font-extralight text-orange-100" />
	</div>
);
		...
		<motion.div
			...
			<motion.div
				...
				style={{
					...
					background:
						"linear-gradient(180deg, #66C9CF 0%, #9CCCB2 100%)",
				}}>
				<svg
					width="24"
					height="24"
...
const MailOpenIcon = () => (
	<div class="bg-default rounded-full p-3">
		<MailOpen class="text-emphasis h-12 w-12 flex-shrink-0 p-0.5 font-extralight" />
	</div>
);
...
export default function Verify(props: inferSSRProps<typeof getServerSideProps>) {
  ...
  useEffect(() => {
    const lastSent = new Date(Number.parseInt(`${t}`));
  ...
  return (
    <div class="text-default bg-muted bg-opacity-90 backdrop-blur-md backdrop-grayscale backdrop-filter">
      <Head>
        <title>
      ...
      <div class="flex min-h-screen flex-col items-center justify-center px-6">
        <div class="border-subtle bg-default m-10 flex max-w-2xl flex-col items-center rounded-xl border px-8 py-14 text-left">
          {hasPaymentFailed ? <PaymentFailedIcon /> : sessionId ? <PaymentSuccess /> : <MailOpenIcon />}
          <h3 class="font-cal text-emphasis my-6 text-2xl font-normal leading-none">
            {hasPaymentFailed
              ? "Your payment failed"
              : sessionId
              ? "Payment successful!"
          ...
          {hasPaymentFailed && (
            <p class="my-6">Your account has been created, but your premium has not been reserved.</p>
          )}
          <p class="text-muted dark:text-subtle text-base font-normal">
        ...
        <div class="flex items-center gap-2">
          <p class="text-subtle text-base font-normal ">Don’t seen an email?</p>
          <button
            class={classNames(
              "font-light",
...
function signin({
	...
	return (
		<div class="center mt-10 justify-between space-y-5 text-center align-baseline">
...
CustomError.getInitialProps = async (ctx: AugmentedNextPageContext) => {
	...
	if (res) {
		...
		log.debug(
			`server side logged this: ${
				err?.toString() ?? JSON.stringify(err)
			}`,
		);
...
export default function Error500() {
  ...
  return (
    <div class="bg-subtle flex h-screen">
      ...
      <div class="rtl: bg-default m-auto rounded-md p-10 text-right ltr:text-left">
        <h1 class="font-cal text-emphasis text-6xl">500</h1>
        <h2 class="text-emphasis mt-6 text-2xl font-medium">It&apos;s not you, it&apos;s us.</h2>
        <p class="text-default mb-6 mt-4 max-w-2xl text-sm">{t("something_went_wrong_on_our_end")}</p>
        {searchParams?.get("error") && (
          <div class="mb-8 flex flex-col">
            <p class="text-default mb-4 max-w-2xl text-sm">
              {t("please_provide_following_text_to_suppport")}:
            </p>
            <pre class="bg-emphasis text-emphasis w-full max-w-2xl whitespace-normal break-words rounded-md p-4">
              {searchParams?.get("error")}
              <br />
              <Button
                color="secondary"
                class="mt-2 border-0 font-sans font-normal hover:bg-gray-300"
                StartIcon={Copy}
                onClick={() => {
        ...
        <Button color="secondary" href="javascript:history.back()" class="ml-2">
          {t("go_back")}
        </Button>
      </div>
    </div>
...
function setHeader(ctx: NextPageContext, name: string, value: string) {
	try {
		...
		console.log(
			`Error setting header ${name}=${value} for ${
				ctx.asPath || "unknown asPath"
			}`,
			e,
		);
	}
...
function ConnectAndJoin() {
	...
	return (
		<div class="mx-8 mt-12 block items-start sm:flex">
			{session ? (
				<EmptyScreen
					headline={t("instant_tab_title")}
					Icon={PhoneCall}
					...
					buttonRaw={
						<div class="flex flex-col items-center justify-center	gap-4">
							{meetingUrl ? (
								<div class="text-default flex flex-col items-center gap-2 text-center text-sm font-normal">
									<Trans i18nKey="some_other_host_already_accepted_the_meeting">
										Some other host already accepted the
										meeting. Do you still want to join?
										<Link
...
function UsernameField({
	...
	useEffect(() => {
		...
		async function checkUsername() {
			...
			fetchUsername(debouncedUsername, orgSlug ?? null).then(
				({ data }) => {
	...
	return (
		<div>
			...
			{(!formState.isSubmitting || !formState.isSubmitted) && (
				<div class="text-gray text-default flex items-center text-sm">
					<div class="text-sm ">
						{usernameTaken ? (
							<div class="text-error flex items-center">
								<Info class="mr-1 inline-block h-4 w-4" />
							...
							<div
								...
								<StarIcon class="mr-1 inline-block h-4 w-4" />
								<p>
									{t("premium_username", {
...
export default function Signup({
  ...
  return (
    <div
      ...
      <div class="bg-muted 2xl:border-subtle grid w-full max-w-[1440px] grid-cols-1 grid-rows-1 overflow-hidden lg:grid-cols-2 2xl:rounded-[20px] 2xl:border 2xl:py-6">
        <HeadSeo title={t("sign_up")} description={t("sign_up")} />
        {/* Left side */}
        <div class="ml-auto mr-auto mt-0 flex w-full max-w-xl flex-col px-4 pt-6 sm:px-16 md:px-20 lg:mt-12 2xl:px-28">
          {/* Header */}
          {errors.apiError && (
          ...
          <div class="flex flex-col gap-2">
            ...
            {IS_CALCOM ? (
              <p class="text-subtle text-base font-medium leading-5">{t("cal_signup_description")}</p>
            ) : (
              <p class="text-subtle text-base font-medium leading-5">
                {t("calcom_explained", {
                  appName: APP_NAME,
                })}
              </p>
          ...
          <div class="mt-12">
            <Form
              ...
              <Button
                type="submit"
                class="my-2 w-full justify-center"
            ...
            {token || (!isGoogleLoginEnabled && !isSAMLLoginEnabled) ? null : (
              <div class="mt-6">
                <div class="relative flex items-center">
                  ...
                  <span class="text-subtle leadning-none mx-2 flex-shrink text-sm font-normal ">
                    {t("or_continue_with")}
                  </span>
                  <div class="border-subtle flex-grow border-t" />
                </div>
          ...
          <div class="mt-10 flex h-full flex-col justify-end text-xs">
            <div class="flex flex-col text-sm">
              <div class="flex gap-1">
                <p class="text-subtle">{t("already_have_account")}</p>
                <Link href="/auth/login" class="text-emphasis hover:underline">
                  {t("sign_in")}
                </Link>
              </div>
              <div class="text-subtle ">
                <Trans i18nKey="signing_up_terms">
                  ...
                  <Link
                    class="text-emphasis hover:underline"
        ...
        <div class="border-subtle lg:bg-subtle mx-auto mt-24 w-full max-w-2xl flex-col justify-between rounded-l-2xl pl-4 dark:bg-none lg:mt-0 lg:flex lg:max-w-full lg:border lg:py-12 lg:pl-12">
          {IS_CALCOM && (
            <>
              <div class="-mt-4 mb-6 mr-12 grid w-full grid-cols-3 gap-5 pr-4 sm:gap-3 lg:grid-cols-4">
                <div>
                  <img
                    src="/product-cards/product-of-the-day.svg"
                    class="h-[34px] w-full dark:invert"
              ...
              <div class="mb-6 mr-12 grid w-full grid-cols-3 gap-5 pr-4 sm:gap-3 lg:grid-cols-4">
                <div>
                  <img
                ...
                <div>
                  <img
                    ...
                    class="block h-[54px] w-full dark:hidden"
                    alt="Trustpilot Rating of 4.7 Stars"
                  />
                  <img
                    src="/product-cards/trustpilot-dark.svg"
                    class="hidden h-[54px] w-full dark:block"
                    alt="Trustpilot Rating of 4.7 Stars"
                  />
                </div>
                <div>
                  <img src="/product-cards/g2.svg" class="h-[54px] w-full" alt="G2 Rating of 4.7 Stars" />
                </div>
              </div>
            </>
          )}
          <div class="border-default hidden rounded-bl-2xl rounded-br-none rounded-tl-2xl border border-r-0 border-dashed bg-black/[3%] dark:bg-white/5 lg:block lg:py-[6px] lg:pl-[6px]">
            <img class="block dark:hidden" src="/mock-event-type-list.svg" alt="Cal.com Booking Page" />
            <img
          ...
          <div class="mr-12 mt-8 hidden h-full w-full grid-cols-3 gap-4 overflow-hidden lg:grid">
            {FEATURES.map((feature) => (
              <>
                <div class="max-w-52 mb-8 flex flex-col leading-none sm:mb-0">
                  <div class="text-emphasis items-center">
                    <feature.icon class="mb-1 h-4 w-4" />
                    <span class="text-sm font-medium">{t(feature.title)}</span>
                  </div>
                  <div class="text-subtle text-sm">
                    <p>
                      {t(
...
const OnboardingPage = () => {
	...
	return (
		<div
			...
			<div class="mx-auto py-6 sm:px-4 md:py-24">
				<div class="relative">
					<div class="sm:mx-auto sm:w-full sm:max-w-[600px]">
						<div class="mx-auto px-4 sm:max-w-[520px]">
							<header>
								<p class="font-cal mb-3 text-[28px] font-medium leading-7">
									{headers[currentStepIndex]?.title ||
										"Undefined title"}
								</p>

								{headers[currentStepIndex]?.subtitle.map(
									(subtitle, index) => (
										<p
											class="text-subtle font-sans text-sm font-normal"
											key={index}>
											{subtitle}
										</p>
									),
						...
						{headers[currentStepIndex]?.skipText && (
							<div class="flex w-full flex-row justify-center">
								<Button
									...
									class="mt-8 cursor-pointer px-4 py-2 font-sans text-sm font-medium">
									{headers[currentStepIndex]?.skipText}
								</Button>
							</div>
						)}
...
function TeamPage({
	...
	if (isUnpublished) {
		...
		return (
			<div class="flex h-full min-h-[100dvh] items-center justify-center">
				<UnpublishedEntity
					{...(metadata?.isOrganization || team.parentId
						? { orgSlug: slug }
						: { teamSlug: slug })}
					name={teamName}
				/>
	...
	const EventTypes = ({
		eventTypes,
	}: { eventTypes: NonNullable<(typeof team)["eventTypes"]> }) => (
		<ul class="border-subtle rounded-md border">
			{eventTypes.map((type, index) => (
				<li
					...
					<div class="px-6 py-4 ">
						<Link
							href={{
								pathname: `${isValidOrgDomain ? "" : "/team"}/${
									team.slug
								}/${type.slug}`,
								query: queryParamsToForward,
							}}
							...
							<div class="flex-shrink">
								<div class="flex flex-wrap items-center space-x-2 rtl:space-x-reverse">
									<h2 class=" text-default text-sm font-semibold">
										{type.title}
									</h2>
								</div>
								<EventTypeDescription
	...
	const SubTeams = () =>
		team.children.length ? (
			<ul class="divide-subtle border-subtle bg-default !static w-full divide-y rounded-md border">
				{team.children.map((ch, i) => {
					...
					return (
						<li key={i} class="hover:bg-muted w-full">
							<Link
								href={`/${ch.slug}`}
								class="flex items-center justify-between">
								<div class="flex items-center px-5 py-5">
									<div class="ms-3 inline-block truncate">
										<span class="text-default text-sm font-bold">
											{ch.name}
										</span>
										<span class="text-subtle block text-xs">
											{t("number_member", {
												count: memberCount,
											})}
										</span>
			...
			<div class="space-y-6" data-testid="event-types">
				<div class="overflow-hidden rounded-sm border dark:border-gray-900">
					<div class="text-muted p-8 text-center">
						<h2 class="font-cal text-emphasis mb-2 text-3xl">{` ${t(
							"org_no_teams_yet",
						)}`}</h2>
						<p class="text-emphasis mx-auto max-w-md">
			...
			<HeadSeo
				...
				meeting={{
					...
					profile: {
						name: `${team.name}`,
						image: `${WEBAPP_URL}/${
							team.metadata?.isOrganization ? "org" : "team"
						}/${team.slug}/avatar.png`,
					},
				}}
			/>
			<main class="dark:bg-darkgray-50 bg-subtle mx-auto max-w-3xl rounded-md px-4 pb-12 pt-12">
				<div class="mx-auto mb-8 max-w-3xl text-center">
					...
					<p
						class="font-cal  text-emphasis mb-2 text-2xl tracking-wider"
						...
						{(showMembers.isOn || !team.eventTypes?.length) &&
							(team.isPrivate ? (
								<div class="w-full text-center">
									<h2
										data-testid="you-cannot-see-team-members"
											...
											<div>
												<div class="relative mt-12">
													<div
														...
														aria-hidden="true">
														<div class="border-subtle w-full border-t" />
													</div>
													<div class="relative flex justify-center">
														<span class="dark:bg-darkgray-50 bg-subtle text-subtle px-2 text-sm">
															{t("or")}
														</span>
													</div>
												</div>

												<aside class="dark:text-inverted mt-8 flex justify-center text-center">
													<Button
														color="minimal"
														EndIcon={ArrowRight}
														data-testid="book-a-team-member-btn"
														class="dark:hover:bg-darkgray-200"
														href={{
															pathname: `${
																isValidOrgDomain
																	? ""
...
export default function UpgradePage() {
  ...
  return (
    <Shell hideHeadingOnMobile={true}>
...
import {
	NewScheduleButton,
	ScheduleListItem,
} from "@calcom/features/schedules";
import { ShellMain } from "@calcom/features/shell/Shell";
...
import type { RouterOutputs } from "@calcom/trpc/react";
import { trpc } from "@calcom/trpc/react";
import { EmptyScreen, ToggleGroup, showToast } from "@calcom/ui";
import { Clock } from "@calcom/ui/components/icon";
import { useAutoAnimate } from "@formkit/auto-animate/react";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
...
export function AvailabilityList({
	schedules,
}: RouterOutputs["viewer"]["availability"]["list"]) {
	const { t } = useLocale();
	const utils = trpc.useContext();
	...
	const deleteMutation = trpc.viewer.availability.schedule.delete.useMutation(
		{
			onMutate: async ({ scheduleId }) => {
				...
				if (previousValue) {
					const filteredValue = previousValue.schedules.filter(
						({ id }) => id !== scheduleId,
					);
					utils.viewer.availability.list.setData(undefined, {
						...previousValue,
						schedules: filteredValue,
					});
				}
			...
			onError: (err, variables, context) => {
				if (context?.previousValue) {
					utils.viewer.availability.list.setData(
						undefined,
						context.previousValue,
					);
				}
	...
	const duplicateMutation =
		trpc.viewer.availability.schedule.duplicate.useMutation({
			onSuccess: async ({ schedule }) => {
				...
				showToast(
					t("schedule_created_successfully", {
						scheduleName: schedule.name,
					}),
					"success",
				);
	...
	return (
		<>
			{schedules.length === 0 ? (
				...
				<>
					<div class="border-subtle bg-default overflow-hidden rounded-md border">
						<ul
							class="divide-subtle divide-y"
							data-testid="schedules"
							ref={animationParentRef}>
							{schedules.map((schedule) => (
								<ScheduleListItem
									displayOptions={{
										hour12: meQuery.data?.timeFormat
											? meQuery.data.timeFormat === 12
											: undefined,
					...
					<div class="text-default mb-16 mt-4 hidden text-center text-sm md:block">
	...
	return (
		<>
			<HeadSeo
				...
				description={t("troubleshoot_availability")}
			/>
...
const DateOverride = ({ workingHours }: { workingHours: WorkingHours[] }) => {
	...
	const excludedDates = fields.map((field) =>
		dayjs(field.ranges[0].start).utc().format("YYYY-MM-DD"),
	);
	const { t } = useLocale();
	return (
		<div class="p-6">
			<h3 class="text-emphasis font-medium leading-6">
				{t("date_overrides")}{" "}
			...
			<p class="text-subtle mb-4 text-sm">
    ...
    <Shell
      ...
      subtitle={
        schedule ? (
          ...
          <SkeletonText class="h-4 w-48" />
        )
      }
      CTA={
        <div class="flex items-center justify-end">
          <div class="sm:hover:bg-muted hidden items-center rounded-md px-2 sm:flex">
            <Skeleton
              as={Label}
              for="hiddenSwitch"
              class="mt-2 cursor-pointer self-center pe-2"
          ...
          <VerticalDivider class="hidden sm:inline" />
          <Dialog>
            <DialogTrigger asChild={true}>
              <Button
                StartIcon={Trash}
          ...
          <div
            ...
            <div
              ...
              <div class="flex flex-row items-center pt-5">
                ...
                <p class="-ml-2">{t("availability_settings")}</p>
                <Dialog>
                  <DialogTrigger asChild={true}>
                    <Button
                      StartIcon={Trash}
              ...
              <div class="flex h-9 flex-row-reverse items-center justify-end gap-3 px-2">
                <Skeleton
                  as={Label}
                  for="hiddenSwitch"
                  class="mt-2 cursor-pointer self-center pr-2 sm:inline">
                  {t("set_to_default")}
                </Skeleton>
                <Switch
                  id="hiddenSwitch"
              ...
              <div class="min-w-40 col-span-3 space-y-2 px-2 py-4 lg:col-span-1">
                <div class="xl:max-w-80 w-full pr-4 sm:ml-0 sm:mr-36 sm:p-0">
                  <div>
                    <Skeleton as={Label} for="timeZone" class="mb-0 inline-block leading-none">
                      {t("timezone")}
                    </Skeleton>
                    <Controller
                      control={form.control}
                      ...
                      render={({ field: { onChange, value } }) =>
                        value ? (
                          <TimezoneSelect
                            value={value}
                            class="focus:border-brand-default border-default mt-1 block w-72 rounded-md text-sm"
                            onChange={(timezone) => onChange(timezone.value)}
                          />
                        ) : (
                          <SelectSkeletonLoader class="mt-1 w-72" />
                        )
                      }
                    />
                  </div>
                  <hr class="border-subtle my-7" />
                  <div class="rounded-md md:block">
                    <Skeleton as="h3" class="mb-0 inline-block text-sm font-medium">
      ...
      <div class="mt-4 w-full md:mt-0">
        <Form
          ...
          class="flex flex-col sm:mx-0 xl:flex-row xl:space-x-6">
          <div class="flex-1 flex-row xl:mr-0">
            <div class="border-subtle mb-6 rounded-md border">
              <div>
            ...
            <div class="border-subtle my-6 rounded-md border">
              {schedule?.workingHours && <DateOverride workingHours={schedule.workingHours} />}
            </div>
          </div>
          <div class="min-w-40 col-span-3 hidden space-y-2 md:block lg:col-span-1">
            <div class="xl:max-w-80 w-full pr-4 sm:ml-0 sm:mr-36 sm:p-0">
              <div>
                <Skeleton as={Label} for="timeZone" class="mb-0 inline-block leading-none">
                  {t("timezone")}
              ...
              <hr class="border-subtle my-6 mr-8" />
...
export {
	getServerSideProps,
	type UserPageProps,
} from "~/users/views/users-public-view.getServerSideProps";
...
export const getServerSideProps = (context: GetServerSidePropsContext) => {
	...
	return {
		redirect: {
			destination: `/apps/routing-forms/${
...
function AppsSearch({
	...
	return (
		<TextField
			class="bg-subtle !border-muted !pl-0 focus:!ring-offset-0"
			addOnLeading={<Search class="text-subtle h-4 w-4" />}
			addOnClassname="!border-muted"
			containerClassName={classNames(
				"focus:!ring-offset-0 m-1",
				className,
			)}
			type="search"
			autoComplete="false"
...
export default function Apps({
  ...
  return (
    <AppsLayout
      ...
      actions={(className) => (
        <div class="flex w-full flex-col pt-4 md:flex-row md:justify-between md:pt-0 lg:w-auto">
          <div class="ltr:mr-2 rtl:ml-2 lg:hidden">
            <HorizontalTabs tabs={tabs} />
          </div>
          <div>
            <AppsSearch class={className} onChange={(e) => setSearchText(e.target.value)} />
          </div>
        </div>
      )}
	...
	return (
		<QueryCell
			...
			success={({ data }) => {
				...
				return (
					<div class="border-subtle rounded-md border p-7">
						<ShellSubHeading
...
export default function Apps({ categories }: Omit<inferSSRProps<typeof getServerSideProps>, "trpcState">) {
  ...
  return (
    <Shell isPublic={true} large={true} hideHeadingOnMobile={true}>
      <div class="text-md flex items-center gap-1 px-4 pb-3 pt-3 font-normal md:px-8 lg:px-0 lg:pt-0">
        <Link
          href="/apps"
          class="text-emphasis inline-flex items-center justify-start gap-1 rounded-sm py-2">
          <ArrowLeft class="h-4 w-4" />
          {isLocaleReady ? t("app_store") : <SkeletonText class="h-6 w-24" />}{" "}
        </Link>
      </div>
      <div class="mb-16">
        <div class="grid h-auto w-full grid-cols-5 gap-3">
          {categories.map((category) => (
            <Link
              key={category.name}
              href={`/apps/categories/${category.name}`}
              data-testid={`app-store-category-${category.name}`}
              class="bg-subtle relative flex rounded-sm px-6 py-4 sm:block">
              <div class="self-center">
                <h3 class="font-medium capitalize">{category.name}</h3>
                <p class="text-subtle text-sm">
  ...
  return (
    <>
      <Shell
        ...
        heading={
          <>
            <Link
              href="/apps"
              class="text-emphasis inline-flex items-center justify-start gap-1 rounded-sm py-2">
              {isLocaleReady ? t("app_store") : <SkeletonText class="h-4 w-24" />}{" "}
            </Link>
            {category && (
              <span class="text-default gap-1">
                <span>&nbsp;/&nbsp;</span>
        ...
        <div class="mb-16">
          <div class="grid-col-1 grid grid-cols-1 gap-3 md:grid-cols-3">
            {apps
...
function SingleAppPage(props: inferSSRProps<typeof getStaticProps>) {
	...
	if (props.isAppDisabled) {
		if (!IS_PRODUCTION) {
			...
			return (
				<div class="p-2">
					This App seems to be disabled. If you are an admin, you can
					enable this app from{" "}
					<Link
		...
		<App
			...
			body={
				<>
					<div
						dangerouslySetInnerHTML={{
							__html: md.render(source.content),
						}}
					/>
				</>
			}
...
export default function SetupInformation(props: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  if (status === "loading") {
    return <div class="bg-emphasis absolute z-50 flex h-screen w-full items-center" />;
  }
...
function MyApp(props: AppProps) {
	...
	if (Component.PageWrapper !== undefined)
		return Component.PageWrapper(props);
	return <Component {...pageProps} />;
}
...
MyApp.getInitialProps = async (ctx: AppContextType) => {
	...
	if (req) {
		const { getLocale } = await import(
			"@calcom/features/auth/lib/getLocale"
		);
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
	...
	const { uid, seatReferenceUid: maybeSeatReferenceUid } =
	...
	const eventType = booking.eventType
		? booking.eventType
		: getDefaultEvent(dynamicEventSlugRef);
  ...
  return (
    <Shell hideHeadingOnMobile={true}>
      <div class="max-w-screen-lg">
        ...
        <p class="text-subtle mt-6 text-xs leading-tight md:hidden">{t("more_page_footer")}</p>
      </div>
    </Shell>
  );
}
